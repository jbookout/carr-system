...
enforced a second time inside ops.clear_recovered_incident (migration 0286),
...
MIGRATION = os.path.join(
    REPO, "migrations", "0286_incident_fingerprint_and_success_clears.sql")
...
def _(assert_):
    # Migration 0286 restates the exit-code meanings so a one-time backfill can
    ...
    block = re.search(r"values\s*(\(\s*64\b.*?)\)\s*as m\(code, class\)", sql,
                      re.S | re.I)
    assert_(block is not None, "0286 no longer carries the exit-code backfill table")
    if block is None:
        return
    pairs = {int(c): n for c, n in
             re.findall(r"\(\s*(\d+)\s*,\s*'([a-z_]+)'\s*\)", block.group(1))}
    assert_(pairs == opsrec.NAMED_EXIT_CLASSES,
            f"0286 and NAMED_EXIT_CLASSES disagree:\n  sql:    {pairs}\n"
