...
export function renderPolicyEpochMigration(registrySql) {
...
begin
  ...
  if observed_count<>209 or observed_digest<>'sha256:b0ad6b4494025998f29a5476f6dd4ed8d6a433426db7677f86d8e44d7171cfe7' then return false; end if;
  ...
  select count(*),'sha256:'||encode(public.digest(convert_to(ops.scac_canonical_json(coalesce(jsonb_agg(row order by ingress_key),'[]'::jsonb)),'UTF8'),'sha256'),'hex') into observed_count,observed_digest from observed;
  if observed_count<>284 or observed_digest<>'sha256:3bb06a15f3f19914d476edd5a2c789e307b5298633c2d4d98c1a3e5c10359345' then return false; end if;
