...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 64)}
...
assert "0575_answer_needs_joe_work_request_scac_successor.sql" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "62", "63"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 62
assert loaded_sql.count("sha256:") == 62
assert FULL_SET_SEALS["scac-mutation-registry.v62"] in loaded_sql, (
