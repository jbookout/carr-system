...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 36)}
...
assert "ops.scac_mutation_catalog_v35_current()" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "34", "35"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 34
assert loaded_sql.count("sha256:") == 34
assert FULL_SET_SEALS["scac-mutation-registry.v34"] in loaded_sql, (
...
def loader_rejects(seals: dict, count: str, current: str = "35") -> bool:
...
runtime_seals = {name: runtime_seal(RUNTIME_V35, name) for name in (
