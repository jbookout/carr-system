...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 89)}
...
assert "0729_provision_engineering_controller_scac_successor.sql" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "87", "88"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 87
assert loaded_sql.count("sha256:") == 87
assert FULL_SET_SEALS["scac-mutation-registry.v87"] in loaded_sql, (
