...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 45)}
...
assert "SCAC_FULL_SET_SEAL_COUNT=43" in GENERATOR
assert "ops.scac_mutation_catalog_v44_current()" in GENERATOR
assert "0550_deal_conflict_scac_successor.sql" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "43", "44"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 43
assert loaded_sql.count("sha256:") == 43
assert FULL_SET_SEALS["scac-mutation-registry.v41"] in loaded_sql, (
    "the newest sealed history must actually reach the SQL the snapshot embeds"
)

# NEGATIVE HALF. The positive above shows the loader renders all 40 sealed rows; it
...
def loader_rejects(seals: dict, count: str, current: str = "44") -> bool:
