...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 75)}
...
assert "0615_global_boundaries_door_scac_successor.sql" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "73", "74"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 73
assert loaded_sql.count("sha256:") == 73
assert FULL_SET_SEALS["scac-mutation-registry.v73"] in loaded_sql, (
