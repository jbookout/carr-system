...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 54)}
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "52", "53"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 52
assert loaded_sql.count("sha256:") == 52
assert FULL_SET_SEALS["scac-mutation-registry.v52"] in loaded_sql, (
...
def loader_rejects(seals: dict, count: str, current: str = "53") -> bool:
