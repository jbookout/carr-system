...
RUNTIME_V33 = (ROOT / "mcp-server" / "src" / "scac-mutation-registry.v33.generated.js").read_text(
    encoding="utf-8"
)
...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 34)}
...
assert "ops.scac_mutation_catalog_v33_current()" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "32", "33"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 32
assert loaded_sql.count("sha256:") == 32
assert FULL_SET_SEALS["scac-mutation-registry.v32"] in loaded_sql, (
...
def loader_rejects(seals: dict, count: str, current: str = "33") -> bool:
...
runtime_seals = {name: runtime_seal(RUNTIME_V33, name) for name in (
