...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 37)}
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "35", "36"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 35
assert loaded_sql.count("sha256:") == 35
assert FULL_SET_SEALS["scac-mutation-registry.v35"] in loaded_sql, (
...
def loader_rejects(seals: dict, count: str, current: str = "36") -> bool:
...
runtime_seals = {name: runtime_seal(RUNTIME_V36, name) for name in (
