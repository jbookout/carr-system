...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 74)}
...
assert "0607_delivery_cadence_a05_scac_successor.sql" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "72", "73"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 72
assert loaded_sql.count("sha256:") == 72
assert FULL_SET_SEALS["scac-mutation-registry.v72"] in loaded_sql, (
