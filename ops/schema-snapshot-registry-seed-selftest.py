...
RUNTIME_V27 = (ROOT / "mcp-server" / "src" / "scac-mutation-registry.v27.generated.js").read_text(
    encoding="utf-8"
)
...
assert set(FULL_SET_SEALS) == {f"scac-mutation-registry.v{version}" for version in range(1, 28)}
...
# The v27 frontier the WR95 successor installs. Pinned BEFORE the predecessor
# branches below so this file fails if the snapshot ever loses the current
...
assert "SCAC_TOTAL_ENTRY_COUNT=40447" in GENERATOR
assert "SCAC_CURRENT_ENTRY_COUNT=1776" in GENERATOR
...
loaded_sql = subprocess.run(
    ["node", "-e", loader, str(ROOT / "ops" / "config" / "scac-registry-full-entry-set-seals.json"), "26", "27"],
    check=True,
    capture_output=True,
    text=True,
).stdout
assert loaded_sql.count("scac-mutation-registry.v") == 26
assert loaded_sql.count("sha256:") == 26
assert FULL_SET_SEALS["scac-mutation-registry.v26"] in loaded_sql, (
    "the newest sealed history must actually reach the SQL the snapshot embeds"
)

# NEGATIVE HALF. The positive above shows the loader renders all 26 sealed rows; it
...
def loader_rejects(seals: dict, count: str, current: str = "27") -> bool:
...
dropped = {k: v for k, v in FULL_SET_SEALS.items() if k != "scac-mutation-registry.v26"}
assert loader_rejects(dropped, "26"), "a seal file missing v26 must not load"
assert loader_rejects(dropped, "21"), (
    "lowering the count must not be a way to hide a missing v26 seal"
)
malformed = dict(FULL_SET_SEALS, **{"scac-mutation-registry.v26": "sha256:not-a-digest"})
assert loader_rejects(malformed, "26"), "a malformed v26 seal must not load"
...
runtime_seals = {name: runtime_seal(RUNTIME_V27, name) for name in (
