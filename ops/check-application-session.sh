...
# touching the substrate, and after any change to migration 0232.
#
...
"$PYBIN" -c "import psycopg" 2>/dev/null || {
  ...
  exit 1; }
MIGRATION="$REPO/migrations/0232_authenticated_application_session.sql"
...
MIGRATION_ISSUER="$REPO/migrations/0233_session_issuer_credential.sql"
...
MIGRATION_SLUG_MINT="$REPO/migrations/0234_mint_session_by_actor_slug.sql"
...
MIGRATION_RECEIPT="$REPO/migrations/0235_write_receipt.sql"
# 0236 introduces the reducer and the acceptance surface — deliberately, and
# only after receipts can prove themselves.
MIGRATION_ACCEPT="$REPO/migrations/0236_continuity_reducer_and_acceptance.sql"
...
MIGRATION_RETIRE="$REPO/migrations/0237_drive_retirement.sql"
...
MIGRATION_SPLIT="$REPO/migrations/0238_receipt_digest_split.sql"
...
echo "migration 0232 applied (its own apply-time assertions passed)"
psql "$BASE/subject" -v ON_ERROR_STOP=1 -q -f "$MIGRATION_ISSUER"
echo "migration 0233 applied (the minting credential is chosen and asserted)"
psql "$BASE/subject" -v ON_ERROR_STOP=1 -q -f "$MIGRATION_SLUG_MINT"
echo "migration 0234 applied (the door can mint without an actor id)"
psql "$BASE/subject" -v ON_ERROR_STOP=1 -q -f "$MIGRATION_RECEIPT"
echo "migration 0235 applied (receipts bind a session and prove by readback)"
psql "$BASE/subject" -v ON_ERROR_STOP=1 -q -f "$MIGRATION_ACCEPT"
echo "migration 0236 applied (the reducer folds, and acceptance is gated)"
psql "$BASE/subject" -v ON_ERROR_STOP=1 -q -f "$MIGRATION_RETIRE"
echo "migration 0237 applied (retirement needs two proven receipts and authority)"
psql "$BASE/subject" -v ON_ERROR_STOP=1 -q -f "$MIGRATION_SPLIT"
echo "migration 0238 applied (the call digest and the material claim are two columns)"
