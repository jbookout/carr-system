...
def main() -> int:
    ...
    check("forward catalog repair is additive and rejects drift before sync",
          ...
          and "0228 failed: control catalog does not match the two exact reviewed controls" in catalog_seed
          and catalog_seed.index("on conflict (control_key) do nothing")
              < catalog_seed.index("0228 failed: control catalog does not match the two exact reviewed controls")
    ...
    # The CONTROL stays registered because it still guards reviewed release
    # wrappers; only the retired rule-to-control binding is absent.
    check("the retired cost rule is not bound by the migration",
          "a57d981a-8f6d-4c18-95ee-0e63a5a90b89" not in executable_sql)
    check("forward scope repair pins the governance rule without rewriting it",
          bool(scope_fix)
          ...
          and "a57d981a-8f6d-4c18-95ee-0e63a5a90b89" not in scope_fix
