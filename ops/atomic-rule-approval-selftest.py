...
MIGRATION = REPO / "migrations" / "0226_atomic_rule_lifecycle_forward_upgrade.sql"
...
def main() -> int:
    ...
    check("forward catalog repair is additive and rejects drift before sync",
          ...
          and "0226 failed: control catalog does not match the two exact reviewed controls" in catalog_seed
          and catalog_seed.index("on conflict (control_key) do nothing")
              < catalog_seed.index("0226 failed: control catalog does not match the two exact reviewed controls")
          and catalog_seed.index("0226 failed: control catalog does not match the two exact reviewed controls")
