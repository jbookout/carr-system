...
# Unique per process, for the same reason chat-lint-gate-selftest.py's is: out/
# is a symlink back to the canonical checkout in every worktree on this Mac, so
# a fixed id is shared by every concurrent ops/ci.sh run. Every blocked fixture
# here reaches remember_blocked(), which writes out/conduct-gate-blocked/<id>.json.
# Nothing in this file reads that record back today — all three helpers send
# stop_hook_active=False, and the verdicts come from the hook's stdout — so this
# is a latent hazard rather than the live flake, but the first fixture that
# exercises the resend path would inherit the same cross-wire.
...
if __name__ == "__main__":
    try:
        ...
        # Every blocked fixture leaves a delta record behind; it is per-process
        # now, so it can never cross wires, but it would still litter the shared
        # out/ once per CI run.
        try:
            os.unlink(os.path.join(REPO, "out", "conduct-gate-blocked",
                                   f"{SESSION}.json"))
        except Exception:
            pass
    sys.exit(rc)
