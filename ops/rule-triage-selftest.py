...
WHAT THIS PINS, per the slice's own definition of done:
  1. 219/219 coverage — every id in ops/config/rule-enforcement-map.json's
  ...
  5. Every GONE rule names a merge_target that (a) exists among the 219 ids
...
def main() -> int:
    ...
    # 1. coverage
    check("219 expected active rules in the enforcement map",
          len(expected_ids) == 219, f"found {len(expected_ids)}")
    ...
    check("triage covers every id the enforcement map knows (219/219)",
    ...
    check("every GONE rule's merge_target exists among the 219 ids",
