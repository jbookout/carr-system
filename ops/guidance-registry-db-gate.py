...
FUNCTIONS = (
    ...
    "ops.activate_guidance_registry(uuid,uuid,text,text)",
)
...
def revision(cur: psycopg.Cursor[Any], item_id: Any, actor_id: Any, kind: str = "procedure") -> Any:
    ...
    delivery: dict[str, Any] = {"projection": "procedure_workflow"}
    if kind == "doctrine":
    ...
    return one(cur, """insert into ops.guidance_revision
        (guidance_item_id,version,guidance_type,scope,activation,consumer,
         verification,provenance,delivery,classified_by,reason)
        values (%s,1,%s,%s,%s,'database acceptance gate',%s,
            ...
            actor_id, "rollback-only fixture"))
        ...
        with psycopg.connect(dsn) as conn:
            with conn.cursor() as cur:
                ...
                if one(cur, "select count(*) from ops.assert_guidance_registry_coverage()") == 0:
                    fail("coverage unexpectedly passes without a reviewed full migration")
                registry_id = one(cur, "select id from ops.guidance_registry where singleton")
                refuses(cur, "select ops.activate_guidance_registry(%s,%s,%s,%s)",
                         (registry_id, uuid.uuid4(), "0" * 64, "must refuse incomplete registry"),
                         "registry activation without valid coverage/authority")
            conn.rollback()
