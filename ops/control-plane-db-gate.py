...
def main() -> int:
    ...
    with rollback_only_connection(dsn) as conn:
        with conn.cursor() as cur:
            ...
            # omitted its mutable catalog seeds. 0222 must restore exactly the
            ...
            if fetchone_required(cur.fetchone(), "forward control catalog restoration")[0] != 2:
                fail("0222 did not restore the two exact reviewed control catalog rows")
            ...
            # 0222 may preserve an OLD 0194 pre-activation receipt only through
            # its exact migration-time anchor. A fresh post-0222 approval is
