...
def main() -> int:
    ...
    try:
        with rollback_only_connection(dsn) as conn, conn.cursor() as cur:
            registry = cur.execute(
                """select registry_digest,entry_count,source_entry_count,catalog_projection,
                    ...
                    where registry_version='scac-mutation-registry.v19'"""
            ).fetchone()
            if registry is None or registry[4:] != (False, False, False):
                raise RuntimeError(f"v19 registry is absent or authority-expanding: {registry!r}")
            if registry[3].get("runtime_dml_grants") != {
                "count": EXPECTED_GRANT_COUNT, "digest": EXPECTED_GRANT_DIGEST,
            }:
                raise RuntimeError(f"v19 grant projection is not exact: {registry[3]!r}")
            ...
            if epoch_chain.get("valid") is not True or \
               ...
               epoch_chain.get("registry_version") != "scac-mutation-registry.v19" or \
               epoch_chain.get("registry_digest") != registry[0] or \
               epoch_chain.get("current_source_digest") != epoch_chain.get("live_source_digest"):
                raise RuntimeError(f"real v19 policy epoch chain is not current: {epoch_chain!r}")
            ...
            if state.get("monitor_state") != "current" or \
               ...
               state.get("registry_version") != "scac-mutation-registry.v19" or \
            ...
            lookup = cur.execute(
                "select ops.scac_mutation_registration_v19(%s,'mcp-tool:standing-context')",
                (registry[0],),
            ).fetchone()[0]
            if lookup.get("registered") is not True or \
               lookup.get("registry_version") != "scac-mutation-registry.v19":
                raise RuntimeError(f"v19 exact registry lookup refused: {lookup!r}")
            if cur.execute(
            ...
            # no longer enumerates platform/superuser roles, so the v19 catalog
            ...
            if cur.execute("select ops.scac_mutation_catalog_v19_current()").fetchone()[0] is not True:
                raise RuntimeError("v19 catalog not current before escalation mutation")
            ...
            if cur.execute("select ops.scac_mutation_catalog_v19_current()").fetchone()[0] is not False:
                raise RuntimeError(
            ...
            # also fail the v19 current-check closed, with no exception for any
            # carr_ role including the one named in the now-removed carve-out.
            if cur.execute("select ops.scac_mutation_catalog_v19_current()").fetchone()[0] is not True:
                raise RuntimeError("v19 catalog not current before superuser-bundle mutation")
            ...
            if not isinstance(snapshot, dict) or snapshot.get("registry_version") != "scac-mutation-registry.v19":
                raise RuntimeError(
                    f"ops.scac_policy_epoch_snapshot() did not succeed on the real, "
                    f"non-bootstrap path with the repaired templates: {snapshot!r}"
                )
            v19_migration_path = REPO / "migrations/0493_incident_work_request_link_scac_successor.sql"
            v19_migration_sql = v19_migration_path.read_text(encoding="utf-8")
            committed_v19_current = current_function_sql(v19_migration_sql, 19)
            committed_v19_body = function_body(committed_v19_current)
            ...
            # under `*_live_at_seal` plus v19 must all carry the narrowed census.
            installed_catalog_functions = cur.execute(
                """select p.proname, pg_get_functiondef(p.oid), p.prosrc
                     from pg_proc p
                     join pg_namespace n on n.oid=p.pronamespace
                    where n.nspname='ops'
                      and p.proname~'^scac_mutation_catalog_v([2-9]|1[0-9])_(current|live_at_seal)$'
                    order by p.proname"""
            ).fetchall()
            installed_names = {row[0] for row in installed_catalog_functions}
            expected_current_names = {
                f"scac_mutation_catalog_v{version}_current" for version in range(2, 20)
            }
            if not expected_current_names.issubset(installed_names):
                raise RuntimeError(
                    "canonical migration chain did not install every v2-v19 current "
            ...
            if len(installed_validators) != 16:
                raise RuntimeError(
                    "canonical migration chain did not retain the expected sixteen "
                    f"role-authority validators (v4-v19): {[row[0] for row in installed_validators]!r}"
                )
            ...
            installed_v19 = next(
                ...
                if row[0] == "scac_mutation_catalog_v19_current"
            )
            installed_v19_definition = installed_v19[1]
            installed_v19_body = installed_v19[2].strip()
            if installed_v19_body != committed_v19_body:
                raise RuntimeError(
                    "installed ops.scac_mutation_catalog_v19_current() body does not "
                    "match the committed 0493 migration definition"
                )
            if NARROWED_ROLE_AUTHORITY_SCOPE not in installed_v19_definition:
                raise RuntimeError(
                    "the installed ops.scac_mutation_catalog_v19_current() definition no "
            ...
            widened_v19_current = installed_v19_definition.replace(
                NARROWED_ROLE_AUTHORITY_SCOPE, UNNARROWED_ROLE_AUTHORITY_SCOPE
            )
            if widened_v19_current == installed_v19_definition:
            ...
            cur.execute(widened_v19_current)
            active_widened_definition = cur.execute(
                "select pg_get_functiondef('ops.scac_mutation_catalog_v19_current()'::regprocedure)"
            ).fetchone()[0]
            ...
            restored_v19_definition = cur.execute(
                "select pg_get_functiondef('ops.scac_mutation_catalog_v19_current()'::regprocedure)"
            ).fetchone()[0]
            if restored_v19_definition != installed_v19_definition:
                raise RuntimeError(
                    "rollback did not restore the committed v19 current-check definition"
                )
            if cur.execute("select ops.scac_mutation_catalog_v19_current()").fetchone()[0] is not True:
                raise RuntimeError("v19 catalog current-check did not re-arm after the census-scope rollback")
        ...
        return fail(str(exc))
    print(
        "siep18-reference-monitor-local-pg-gate passed: exact v19 grant seal, "
