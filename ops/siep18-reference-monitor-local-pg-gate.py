...
# exactly once inside the live ops.scac_mutation_catalog_v13_current()
# definition in migrations/0487_claude_startup_registry_activation.sql --
...
def main() -> int:
    ...
    try:
        with rollback_only_connection(dsn) as conn, conn.cursor() as cur:
            registry = cur.execute(
                """select registry_digest,entry_count,source_entry_count,catalog_projection,
                    ...
                    where registry_version='scac-mutation-registry.v13'"""
            ).fetchone()
            if registry is None or registry[4:] != (False, False, False):
                raise RuntimeError(f"v13 registry is absent or authority-expanding: {registry!r}")
            if registry[3].get("runtime_dml_grants") != {
                "count": EXPECTED_GRANT_COUNT, "digest": EXPECTED_GRANT_DIGEST,
            }:
                raise RuntimeError(f"v13 grant projection is not exact: {registry[3]!r}")
            ...
            if epoch_chain.get("valid") is not True or \
               ...
               epoch_chain.get("registry_version") != "scac-mutation-registry.v13" or \
               epoch_chain.get("registry_digest") != registry[0] or \
               epoch_chain.get("current_source_digest") != epoch_chain.get("live_source_digest"):
                raise RuntimeError(f"real v13 policy epoch chain is not current: {epoch_chain!r}")
            ...
            if state.get("monitor_state") != "current" or \
               ...
               state.get("registry_version") != "scac-mutation-registry.v13" or \
            ...
            lookup = cur.execute(
                "select ops.scac_mutation_registration_v13(%s,'mcp-tool:standing-context')",
                (registry[0],),
            ).fetchone()[0]
            if lookup.get("registered") is not True or \
               lookup.get("registry_version") != "scac-mutation-registry.v13":
                raise RuntimeError(f"v13 exact registry lookup refused: {lookup!r}")
            if cur.execute(
                "select ops.scac_mutation_registry_v12_seal_available()"
            ).fetchone()[0] is not True:
                raise RuntimeError("sealed v12 predecessor is unavailable")
            ...
            # no longer enumerates platform/superuser roles, so the v13 catalog
            ...
            if cur.execute("select ops.scac_mutation_catalog_v13_current()").fetchone()[0] is not True:
                raise RuntimeError("v13 catalog not current before escalation mutation")
            ...
            if cur.execute("select ops.scac_mutation_catalog_v13_current()").fetchone()[0] is not False:
                raise RuntimeError(
            ...
            # also fail the v13 current-check closed, with no exception for any
            # carr_ role including the one named in the now-removed carve-out.
            if cur.execute("select ops.scac_mutation_catalog_v13_current()").fetchone()[0] is not True:
                raise RuntimeError("v13 catalog not current before superuser-bundle mutation")
            ...
            if not isinstance(snapshot, dict) or snapshot.get("registry_version") != "scac-mutation-registry.v13":
                raise RuntimeError(
                    f"ops.scac_policy_epoch_snapshot() did not succeed on the real, "
                    f"non-bootstrap path with the repaired templates: {snapshot!r}"
                )
            v13_migration_path = REPO / "migrations/0487_claude_startup_registry_activation.sql"
            v13_migration_sql = v13_migration_path.read_text(encoding="utf-8")
            committed_v13_current = current_function_sql(v13_migration_sql, 13)
            committed_v13_body = function_body(committed_v13_current)
            ...
            # under `*_live_at_seal` plus v13 must all carry the narrowed census.
            installed_catalog_functions = cur.execute(
                """select p.proname, pg_get_functiondef(p.oid), p.prosrc
                     from pg_proc p
                     join pg_namespace n on n.oid=p.pronamespace
                    where n.nspname='ops'
                      and p.proname~'^scac_mutation_catalog_v([2-9]|10|11|12|13)_(current|live_at_seal)$'
                    order by p.proname"""
            ).fetchall()
            installed_names = {row[0] for row in installed_catalog_functions}
            expected_current_names = {
                f"scac_mutation_catalog_v{version}_current" for version in range(2, 14)
            }
            if not expected_current_names.issubset(installed_names):
                raise RuntimeError(
                    "canonical migration chain did not install every v2-v13 current "
            ...
            if len(installed_validators) != 10:
                raise RuntimeError(
                    "canonical migration chain did not retain the expected ten "
                    f"role-authority validators (v4-v13): {[row[0] for row in installed_validators]!r}"
                )
            ...
            installed_v13 = next(
                ...
                if row[0] == "scac_mutation_catalog_v13_current"
            )
            installed_v13_definition = installed_v13[1]
            installed_v13_body = installed_v13[2].strip()
            if installed_v13_body != committed_v13_body:
                raise RuntimeError(
                    "installed ops.scac_mutation_catalog_v13_current() body does not "
                    "match the committed 0487 migration definition"
                )
            if NARROWED_ROLE_AUTHORITY_SCOPE not in installed_v13_definition:
                raise RuntimeError(
                    "the installed ops.scac_mutation_catalog_v13_current() definition no "
            ...
            widened_v13_current = installed_v13_definition.replace(
                NARROWED_ROLE_AUTHORITY_SCOPE, UNNARROWED_ROLE_AUTHORITY_SCOPE
            )
            if widened_v13_current == installed_v13_definition:
            ...
            cur.execute(widened_v13_current)
            active_widened_definition = cur.execute(
                "select pg_get_functiondef('ops.scac_mutation_catalog_v13_current()'::regprocedure)"
            ).fetchone()[0]
            ...
            restored_v13_definition = cur.execute(
                "select pg_get_functiondef('ops.scac_mutation_catalog_v13_current()'::regprocedure)"
            ).fetchone()[0]
            if restored_v13_definition != installed_v13_definition:
                raise RuntimeError(
                    "rollback did not restore the committed v13 current-check definition"
                )
            if cur.execute("select ops.scac_mutation_catalog_v13_current()").fetchone()[0] is not True:
                raise RuntimeError("v13 catalog current-check did not re-arm after the census-scope rollback")
        ...
        return fail(str(exc))
    print(
        "siep18-reference-monitor-local-pg-gate passed: exact v13 grant seal, "
