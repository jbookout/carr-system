...
def test_upsert_to_neon() -> None:
    ...
    mig_path = Path(HERE).parent / "migrations" / "0192_session_work.sql"
    sql_text = mig_path.read_text()
    if "version" not in sql_text:
        fail("upsert version column", "migration 0192 has no 'version' column — trg_touch_row will crash")
        return
...
def test_migration_exists() -> None:
    ...
    ok("migration: 0192_session_work.sql exists and contains required DDL (incl. version column)")
