...
case "$GLOBAL_BOUNDARIES_DOOR_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read global boundaries door v75 registry ledger state" >&2; exit 1 ;;
esac
if [ "$GLOBAL_BOUNDARIES_DOOR_REGISTRY_APPLIED" = t ] && [ "$JOURNEY_ONE_CLOCK_DOOR_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: global boundaries door v75 is applied without v74 predecessor" >&2
  exit 1
fi
