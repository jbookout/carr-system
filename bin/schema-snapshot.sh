...
JEV_HANDOFF_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0559_jev_handoff_scac_successor.sql')" \
  2>/dev/null)"
case "$JEV_HANDOFF_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read jev handoff v51 registry ledger state" >&2; exit 1 ;;
esac
if [ "$JEV_HANDOFF_REGISTRY_APPLIED" = t ] && [ "$MACHINE_ROLE_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: jev handoff v51 is applied without v50 predecessor" >&2
  exit 1
fi
