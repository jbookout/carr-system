...
case "$SALESFORCE_RECONCILIATION_RW02_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read Salesforce reconciliation RW02 v87 registry ledger state" >&2; exit 1 ;;
esac
if [ "$SALESFORCE_RECONCILIATION_RW02_REGISTRY_APPLIED" = t ] && [ "$SESSION_TRACE_ARCHIVE_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: Salesforce reconciliation RW02 v87 is applied without v86 predecessor" >&2
  exit 1
fi
