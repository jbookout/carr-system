...
GLOBAL_BOUNDARIES_DOOR_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0615_global_boundaries_door_scac_successor.sql')" \
  2>/dev/null)"
case "$GLOBAL_BOUNDARIES_DOOR_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read global boundaries door v74 registry ledger state" >&2; exit 1 ;;
esac
if [ "$GLOBAL_BOUNDARIES_DOOR_REGISTRY_APPLIED" = t ] && [ "$DEPLOY_WORKER_DO_MIGRATION_RESEAL_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: global boundaries door v74 is applied without v73 predecessor" >&2
  exit 1
fi
