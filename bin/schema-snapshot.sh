...
CALL_MODE_ORIGIN_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0564_call_mode_origin_scac_successor.sql')" \
  2>/dev/null)"
case "$CALL_MODE_ORIGIN_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read call mode origin v56 registry ledger state" >&2; exit 1 ;;
esac
if [ "$CALL_MODE_ORIGIN_REGISTRY_APPLIED" = t ] && [ "$LOGITECH_KEYMAP_OPTOUT_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: call mode origin v56 is applied without v55 predecessor" >&2
  exit 1
fi
