...
V5_A02_RULE_ENFORCEMENT_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0713_a02_rule_enforcement_coverage_scac_successor.sql')" \
  2>/dev/null)"
case "$V5_A02_RULE_ENFORCEMENT_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read V5-A02 rule enforcement v83 registry ledger state" >&2; exit 1 ;;
esac
if [ "$V5_A02_RULE_ENFORCEMENT_REGISTRY_APPLIED" = t ] && [ "$ACTION_CLASS_SUCCESSOR_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: V5-A02 rule enforcement v83 is applied without v82 predecessor" >&2
  exit 1
fi
