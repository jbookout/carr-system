...
case "$V5_SLICE_DONE_MARKER_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read V5 slice done marker v76 registry ledger state" >&2; exit 1 ;;
esac
if [ "$V5_SLICE_DONE_MARKER_REGISTRY_APPLIED" = t ] && [ "$DELIVERY_CADENCE_A05_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: V5 slice done marker v76 is applied without v75 predecessor" >&2
  exit 1
fi
