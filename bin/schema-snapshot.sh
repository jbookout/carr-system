...
case "$DELIVERY_CADENCE_A05_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read delivery cadence A05 v74 registry ledger state" >&2; exit 1 ;;
esac
if [ "$DELIVERY_CADENCE_A05_REGISTRY_APPLIED" = t ] && [ "$DEPLOY_WORKER_DO_MIGRATION_RESEAL_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: delivery cadence A05 v74 is applied without v73 predecessor" >&2
  exit 1
fi
