...
ASSURANCE_HEALTH_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0710_assurance_health_scac_successor.sql')" \
  2>/dev/null)"
case "$ASSURANCE_HEALTH_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read V5-A01 assurance-health v82 registry ledger state" >&2; exit 1 ;;
esac
if [ "$ASSURANCE_HEALTH_REGISTRY_APPLIED" = t ] && [ "$AMEND_CLOSED_LOOP_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: V5-A01 assurance-health v82 is applied without v81 predecessor" >&2
  exit 1
fi
