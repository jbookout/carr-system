...
V5_SLICE_DONE_MARKER_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0620_doctorcre_slice_done_marker_scac_successor.sql')" \
  2>/dev/null)"
case "$V5_SLICE_DONE_MARKER_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read V5 slice done marker v75 registry ledger state" >&2; exit 1 ;;
esac
if [ "$V5_SLICE_DONE_MARKER_REGISTRY_APPLIED" = t ] && [ "$JOURNEY_ONE_CLOCK_DOOR_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: V5 slice done marker v75 is applied without v74 predecessor" >&2
  exit 1
fi
