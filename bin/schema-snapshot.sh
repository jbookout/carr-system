...
case "$V5_A02_RULE_ENFORCEMENT_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read V5-A02 rule enforcement v82 registry ledger state" >&2; exit 1 ;;
esac
if [ "$V5_A02_RULE_ENFORCEMENT_REGISTRY_APPLIED" = t ] && [ "$AMEND_CLOSED_LOOP_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: V5-A02 rule enforcement v82 is applied without v81 predecessor" >&2
  exit 1
fi
