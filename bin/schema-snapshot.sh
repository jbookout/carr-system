...
case "$SESSION_TRACE_ARCHIVE_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read session trace archive v85 registry ledger state" >&2; exit 1 ;;
esac
if [ "$SESSION_TRACE_ARCHIVE_REGISTRY_APPLIED" = t ] && [ "$V5_A03_REVIEW_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: session trace archive v85 is applied without v84 predecessor" >&2
  exit 1
fi
