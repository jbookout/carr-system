...
PROVISION_ENGINEERING_CONTROLLER_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0729_provision_engineering_controller_scac_successor.sql')" \
  2>/dev/null)"
case "$PROVISION_ENGINEERING_CONTROLLER_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read provision engineering controller v88 registry ledger state" >&2; exit 1 ;;
esac
if [ "$PROVISION_ENGINEERING_CONTROLLER_REGISTRY_APPLIED" = t ] && [ "$F05_RULE_CONTEXT_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: provision engineering controller v88 is applied without v87 predecessor" >&2
  exit 1
fi
