...
case "$GOVERNED_CORRESPONDENCE_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read governed correspondence v78 registry ledger state" >&2; exit 1 ;;
esac
if [ "$GOVERNED_CORRESPONDENCE_REGISTRY_APPLIED" = t ] && [ "$F01_RECORD_SOURCE_AUTHORITY_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: governed correspondence v78 is applied without v77 predecessor" >&2
  exit 1
fi
