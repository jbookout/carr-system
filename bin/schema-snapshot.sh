...
CRE_LIFECYCLE_REGISTRY_APPLIED="$("$PSQL" -Atqc \
  "select exists (select 1 from schema_migrations where filename='0631_cre_lifecycle_scac_successor.sql')" \
  2>/dev/null)"
case "$CRE_LIFECYCLE_REGISTRY_APPLIED" in
  t|f) ;;
  *) echo "schema-snapshot: could not read J102 CRE lifecycle v78 registry ledger state" >&2; exit 1 ;;
esac
if [ "$CRE_LIFECYCLE_REGISTRY_APPLIED" = t ] && [ "$F01_RECORD_SOURCE_AUTHORITY_REGISTRY_APPLIED" != t ]; then
  echo "schema-snapshot: J102 CRE lifecycle v78 is applied without v77 predecessor" >&2
  exit 1
fi
