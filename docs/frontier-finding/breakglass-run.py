#!/usr/bin/env python3
"""breakglass-run.py — Artifact C's receipt driver (WR-000046, F01 condition (d)).
...
INVOCATION (exactly as F01 condition (d) and the execution path describe it):
    ...
    CARR_BREAK_GLASS=1 .venv/bin/python tools/db-tap.py --reason "<WR note ref>" \\
        run docs/frontier-finding/breakglass-run.py -- \\
...
the test harness (breakglass-tests.py) drives it against a local disposable
...
produced by gen-breakglass-run.py: one manifest line (canonical JSON) plus the
...
complete, valid, readable JSON document (see breakglass-tests.py's durability
...
try:
    ...
    sys.exit(
        "breakglass-run: psycopg is not installed in this interpreter — "
...
# ── bundle format (shared with gen-breakglass-run.py) ───────────────────────
...
def run(approved_path: Path, receipt_path: Path) -> int:
    ...
    if manifest.get("manifest_version") != MANIFEST_SCHEMA_VERSION:
        sys.exit(f"breakglass-run: unrecognized manifest_version {manifest.get('manifest_version')!r}")
    ...
    print("breakglass-run: digests to compare against the WR-000046 approval note:")
    print(f"  run-script: {run_script_sha256}")
    print(f"  candidate:  {candidate_sha256}")
    print(f"  manifest:   {manifest_sha256}")

    # Defensive re-check. gen-breakglass-run.py already refuses to generate a
    ...
    if migrate.contains_transaction_control(candidate_sql):
        sys.exit("breakglass-run: REFUSED — candidate SQL contains transaction control")

    database_url = os.environ.get("DATABASE_URL")
    if not database_url:
        sys.exit("breakglass-run: DATABASE_URL is not set")
    ...
    with psycopg.connect(database_url, autocommit=False) as conn:
        ...
        if identity_requirement is not None:
            ...
            if mismatches:
                ...
                conn.rollback()
                print(f"breakglass-run: REFUSED — identity mismatch on {mismatches}", file=sys.stderr)
                return 1
        ...
        if precheck_failures:
            ...
            write_receipt(receipt_path, outcome)
            conn.rollback()
            print("breakglass-run: REFUSED — expected-pre precheck failed before execution", file=sys.stderr)
    ...
    if outcome["verdict"] == "committed":
        print("breakglass-run: COMMITTED")
        if sequence_residuals:
            print(f"breakglass-run: {len(sequence_residuals)} sequence residual(s) — see receipt, not auto-classified")
        return 0
    print("breakglass-run: ROLLED BACK", file=sys.stderr)
    for v in undeclared:
...
def main() -> int:
    ...
    parser.add_argument("--approved", required=True, type=Path, help="the run bundle (see gen-breakglass-run.py)")
