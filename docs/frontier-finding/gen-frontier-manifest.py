#!/usr/bin/env python3
"""gen-frontier-manifest.py — WR-000046 Artifact A, OBSERVED effect manifest.
...
USAGE:
    ...
    python3 gen-frontier-manifest.py run --port 55901 --out /path/run1.json \\
        --apply-log /path/apply-log-run1.txt

    # a second independent run
    python3 gen-frontier-manifest.py run --port 55902 --out /path/run2.json \\
    ...
    python3 gen-frontier-manifest.py reconcile --run-a /path/run1.json \\
...
def run_pipeline(port: int, out_path: Path, apply_log_path: Path, run_tag: str) -> None:
    ...
    log_lines: list[str] = [f"gen-frontier-manifest run={run_tag} port={port} workdir={workdir}"]
...
def reconcile(run_a_path: Path, run_b_path: Path, out_path: Path, result_note_path: Path | None) -> None:
    ...
    manifest = {
        ...
        "provenance": {
            ...
            "generator_path": "docs/frontier-finding/gen-frontier-manifest.py",
