#!/usr/bin/env python3
"""
compare-targets.py -- WR-000046 Artifact A cross-check, comparison seat
(slice-compare).
...
Inputs, all in this directory:
  ...
  - frontier-touched-objects.v1.corrected.json  (produced by
    rerender-observed-comments.py: the SAME artifact with the
    comment:function: rendering bug documented in gen-frontier-manifest.py
  ...
  - parse-extracted-targets.v1.corrected.json  (produced by re-running the
    patched parse-extract-targets.py in this directory against the same
...
def write_report(**ctx):
    ...
    a("")
    a("**3a. PARSED-side patch** (`parse-extract-targets.py` in this directory,")
    ...
    a("   corpus, verified).")
    a("")
    a("**3b. OBSERVED-side patch** (`gen-frontier-manifest.py`'s `diff_comments`")
    ...
    a("`rerender-observed-comments.py`, which reads ONLY data already captured")
    ...
    a("")
    a("Applied by `compare-targets.py` from `target-dispositions.v1.json`. Each")
