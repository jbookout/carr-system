...
assert.equal(marketMap.version, "1.1.0", "AI-assisted map architecture contract version");
...
assert.match(marketMap.ai_assisted_map_build_contract.ai_role, /bounded.*research.*authoring/i, "AI map role is bounded");
assert.match(marketMap.ai_assisted_map_build_contract.forbidden_authority.join(" "), /source of truth.*final geocoder.*route authority.*publish/i, "AI map authority exclusions");
assert.deepEqual(marketMap.ai_assisted_map_build_contract.architecture_layers.map(item => item.id), ["canonical_record", "reviewed_enrichment", "deterministic_route", "interactive_presentation", "native_navigation"]);
assert.ok(marketMap.ai_assisted_map_build_contract.structured_brief.required_fields.includes("verified_stop_dataset"), "structured brief requires verified stops");
assert.ok(marketMap.ai_assisted_map_build_contract.promotion_receipt.required_checks.includes("map_list_route_offline_order_parity"), "promotion receipt requires order parity");
assert.ok(marketMap.ai_assisted_map_build_contract.confirmation_gates.includes("publish_client_or_public_map"), "map publication is confirmation-gated");
