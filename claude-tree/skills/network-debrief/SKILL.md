...
description: >
  ...
  new-lead, set-next-action, link-parties, add-loop) — never by editing generated exports or
  parallel narrative files. The record is the only durable business home.
...
- **Client / prospect activity → the RECORD, via verbs.** `stamp-touch` / `log-activity` for the touch, `set-next-action` for whose ball it is and by when, `new-client` to create one, `update-deal` for deal fields and `set-lead` for deal ownership. Log analysis with `log-activity` kind:`analysis`; do not edit generated client projections or create parallel narrative files.
...
- **Commitments and follow-ups without a single home record — or anything blocking system work → the `add-loop` verb** (owner, what it unblocks; `marker:'bell'` for actionable this week, `'dated'`+`due_on`, `'decision'` for an open question, `'none'` for backlog). Query them with `loop-board`; never edit a generated queue. A settled ruling is `log-decision`.
...
- **Intro outcomes ("those two I connected actually did a deal") → `link-parties` (kind introduced/referred) + `log-activity` on both vendors.** When an intro resolves, also set its Delivery grade (A/B/C/F); use the tap-through question. The grade feeds the Vendor Delivery Scoreboard, which steers future recommendations through active `rule-context` policy.
...
- **Pairing-rule learnings ("never put those two in a room") → the `teach` verb, scope `intro_politics`**, in his verbatim words; propose activation for his yes. Read active pairing policy with `rule-context`.
- **Vendor-candidate verdicts ("yes connect with that CPA" / "pass") → `update-vendor` (stage) plus a `log-activity` note carrying the verdict.** Query history from the record; never edit generated ledgers.

Rules to honor: keep the **two brains separate** (vendors vs. clients), linked only by ID. Preserve **Owner / Originated** tags. Before any vendor recommendation or intro, call **`rule-context`** with `scope_kind:intro_politics`. **Never scrape Dell's SharePoint sheet** to reconcile.
