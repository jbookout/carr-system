...
- **Write is required** for the terms JSON and the post-mortem file. **Read either one before overwriting it.** If a file for this deal already exists, read it and merge rather than replace: a second pass on the same close otherwise discards the first pass's figures silently. Claude Code 2.1.228 dropped the tool-level rule that used to force that read, so the discipline lives here now and nowhere else.
