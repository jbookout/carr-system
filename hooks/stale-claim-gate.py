...
AND THE MOMENT WAS WRONG FOR THIS HALF. drift-claim-gate is PreToolUse on
record-defect and add-loop, so it speaks only when a claim reaches the record.
Eight of the ten rows never reached a write verb: the claim went to Joe in chat
and the cost was paid there. Row 10 proves it — the gate fired correctly on the
record-defect call, but the session had already promised Joe a worktree and a
pull request for work that was merged three hours earlier. Chat is the surface,
so Stop is the moment, the same argument chat-lint-gate.py makes.
