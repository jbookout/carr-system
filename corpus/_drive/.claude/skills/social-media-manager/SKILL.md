...
- `Marketing/Brand Assets/asset-checklist.md` — check what real logo/photo assets exist before defaulting to a generated graphic.
...
**Adversarial batch check (added July 2, 2026, batch refreshes only):** `write-content`'s own Pre-Delivery Self-Check is run by the same context that just wrote each post — fine for a single one-off post, since Joe reviews it directly before anything publishes. A full batch refresh (10+ rows at once) is the higher-risk case: more volume, less per-item attention from Joe on any single row, and the same context grading its own work across every row rather than a fresh set of eyes on each one. Once every row in the batch has been drafted through `write-content`, **suggest to Joe that this batch get one adversarial pass from a dispatched subagent** before presenting it — the subagent's only job is to re-run the Pre-Delivery Self-Check criteria (Credibility Test, Voice Shaper, Pattern Breaker, Read-Aloud Test, Artificial Sentence Check) against each row with fresh context, and flag anything that reads as generic, templated, or off-voice. This is a suggestion to make, not a rule to enforce automatically — per the subagent-delegation standing rule in ai-operating-notes.md, propose it and let Joe decide, don't spawn it unprompted. Source: an X article on "Loop Engineering" (verified July 2, 2026) making the same point Karpathy's LOOPS.md already made in sprint-system-reference.md — the one who writes should never be the one who grades. Treat this as optional extra rigor for batches specifically, not yet a confirmed fix for an actual observed problem, since no full batch has run through the reconciled write-content/social-media-manager split yet. Provenance upgraded July 12, 2026: Anthropic's Code with Claude 2026 conference confirmed this is how they run their own shipping — every PR gets a multi-agent review pass (independent reviewers, then a verification phase on the findings), and the Bun team's take was that high-volume automation doesn't hold up without an independent review in the loop. The writer-never-grades principle is now corroborated practice, not one X article.

**Batch review surface (added July 12, 2026, same conference — the "HTML beats markdown for human review" pattern):** when presenting a full batch for Joe's greenlight, render ONE HTML preview page (via Artifact) instead of a markdown wall: each day as a row — card image beside the per-platform captions, with date, platform, and scheduled fire time visible on each. A batch is 25+ caption variants; markdown scrolls, a dense visual page lets Joe catch a wrong card pairing, a broken caption, or an off-brand graphic before the gate instead of after. Same idea applies to design-direction exploration (the "2–4 cheap variants" rule in ai-operating-notes): generate the variants as side-by-side HTML, not descriptions.
...
- **"Publish [post]"** → run the publish flow below. **Never call the Blotato create-post step without an explicit go-ahead from Joe on that specific post's final copy and visual** — this is a real, public, hard-to-reverse action. **Scheduling rule (added July 3, 2026):** if a post is *scheduled* to fire later rather than published immediately, open a loop with the `add-loop` verb the moment it's scheduled (`00_Context/open-loops.md` is a GENERATED render — editing it loses the entry) — platform, topic, and exact fire date/time, owned by Joe ("review before it fires") — and close that line only after Joe has reviewed it or it has published and been logged. This applies to BOTH paths (this skill's scripts and the Cowork Blotato MCP). Every session reads open-loops.md, so nothing scheduled can fire unreviewed and silently.

### 2. Visual creation (Canva MCP)
1. Check `Marketing/Brand Assets/` for a real logo file or approved photo first — prefer a real asset over a recreated one, per the workflow doc.
