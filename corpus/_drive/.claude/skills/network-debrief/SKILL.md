...
description: >
  ...
  networking brain so the system stays current and the Monday brief reflects reality. Updates
  DNA/Network/vendors.md and DNA/Network/deals.md (vendor side) and the client brain (00_Context/prospects.md
  + Prospects/) — kept separate, linked by ID. Use it whenever Joe wants to recap or document what
  ...
  prospect file (and DNA/templates.md as a candidate rule), and unowned commitments to 00_Context/open-loops.md —
...
- **Client / prospect activity → the client brain:** update `Prospects/<name>.md` and the `00_Context/prospects.md` active row (status, last touch, next step). New prospects get a `C-###` id in `Prospects/prospects-roster.md` and, if active, their own detail file. Keep prospects.md short.
- **Deal-stage changes → `DNA/Network/deals.md`** (LOI / CA / Lease) and the client's detail file.
...
- **Objections heard + pitch language that landed or flopped → the prospect's `Prospects/<name>.md` file** (dated note). If the same objection or win shows up more than once, or Joe confirms it generalizes, propose it as a rule/hook change in `DNA/templates.md` — don't silently edit approved voice rules without his yes.
- **Commitments and follow-ups without a single home record — or anything blocking system work → `00_Context/open-loops.md`** (one line, owner, what it unblocks). A promised follow-up to a specific vendor/prospect goes on their record as the next step instead; open-loops is for the rest.
...
- **Joint projects (co-marketing, planned panels, shared booths) → `DNA/Network/deals.md` Joint Projects log** with a next step + owner.
- **Intro outcomes ("those two I connected actually did a deal") → `DNA/Network/deals.md` Intro Outcomes log** — quality-weighted reciprocity depends on this. **When an intro RESOLVES, also set its Delivery grade (A/B/C/F — did the vendor perform, deals.md rubric); use the tap-through (AskUserQuestion) for the grade. That grade feeds the Vendor Delivery Scoreboard, which steers who gets the next client (introduction-rules.md).**
- **New leads from any conversation → `DNA/Leads/lead-registry.xlsx`** (new L-### row, Source Type + vendor V-ID attribution mandatory) — plus prospects.md + a detail file if it's immediately active. The registry is Claude's to maintain; Joe never needs to open it.
...
- **Vendor-candidate verdicts ("yes connect with that CPA" / "pass") → vendors.md 🎯 Targets + hunt ledger.**
