-- 0233 — the credential that may mint an authenticated application session
--
-- WHAT 0232 LEFT OPEN, DELIBERATELY. 0232 created carr_session_minter NOLOGIN
...
-- authenticated session, and every guarantee 0232 enforces is enforced against
...
-- mint a session and then bind rows to it, which is precisely the attack 0232
...
-- nothing to bind to it: 0232's guard requires the row's actor and tenant to
-- match, and the issuer cannot insert the row at all.
revoke all on schema public from carr_session_issuer;

-- --------------------------------------------------------------- apply-time
-- Catalog assertions only, and that is deliberate rather than lazy: 0232's
...
begin
  if not exists (select 1 from pg_roles where rolname='carr_session_issuer' and rolcanlogin) then
    raise exception '0233 FAILED: carr_session_issuer must exist and be able to log in; '
  ...
  if pg_has_role('carr_session_issuer', 'carr_writer', 'MEMBER')
    ...
    raise exception '0233 FAILED: carr_session_issuer can assume a write or '
  ...
  if members is distinct from 'carr_session_issuer' then
    raise exception '0233 FAILED: carr_session_minter members must be exactly '
