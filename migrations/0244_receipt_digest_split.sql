...
-- `bin/migrate-prod.sh --through 0243_drive_retirement.sql` is exactly that --
...
begin
  ...
  if probe_actor is null then
    raise exception '0244 FAILED: need a human actor to exercise the split';
  end if;
  ...
  if ops.write_receipt_digest('log-activity', probe_actor, 'carr-internal', sid,
   ...
   = ops.write_receipt_digest('log-activity', probe_actor, 'carr-internal', sid,
    ...
    raise exception '0244 FAILED: the call digest is identical for two different subjects';
  end if;
  ...
  if not ops.prove_write_receipt(r1) then
    raise exception '0244 FAILED: an honest receipt did not prove after the split';
  end if;
  ...
  exception when others then
    ...
    if position('wrote nothing about that subject' in sqlerrm) = 0 then
      raise exception '0244 FAILED: untouched-subject receipt refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0244 FAILED: a receipt named a subject its call never wrote to';
  end if;
  ...
  exception when others then
    ...
    if position('does not match what its call wrote' in sqlerrm) = 0 then
      raise exception '0244 FAILED: false-material receipt refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0244 FAILED: a receipt carried material its call never wrote';
  end if;
  ...
  begin
    ...
    exception when others then
      ...
      if position('claims verb' in sqlerrm) = 0 then
        raise exception '0244 FAILED: verb mismatch refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a receipt claimed a verb its evidence does not record';
    end if;
  end;
  ...
  if mat_b = mat_a then
    raise exception '0244 FAILED: two DIFFERENT changes hashed to the same material';
  end if;
  insert into ops.write_receipt
    (id, application_session_id, actor_id, organization_tenant_id, verb,
     subject_type, subject_id, tool_call_idempotency_key,
  ...
  if not ops.prove_write_receipt(r2) then
    raise exception '0244 FAILED: the second honest receipt did not prove';
  end if;
  ...
  if not ops.prove_write_receipt(rev) then
    raise exception '0244 FAILED: an EXACT REVERSAL still cannot prove -- the '
  ...
  exception when others then
    ...
    if position('reversal is not exact' in sqlerrm) = 0 then
      raise exception '0244 FAILED: inexact reversal refused by the WRONG guard: %', sqlerrm;
    end if;
    ...
    if position(mat_a in sqlerrm) > 0 then
      raise exception '0244 FAILED: the reversal refusal disclosed its target''s prior state';
    end if;
  end;
  if not failed then
    raise exception '0244 FAILED: an inexact reversal was accepted';
  end if;
  ...
  begin
    ...
    exception when others then
      ...
      if position('same subject as the receipt it reverses' in sqlerrm) = 0 then
        raise exception '0244 FAILED: cross-subject reversal refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a reversal named a different subject than its target';
    end if;
  end;
  ...
  begin
    ...
    if mb4 is distinct from mb2 then
      raise exception '0244 FAILED: the same change written twice hashed differently, '
    ...
    if r.state <> 'continuous' then
      raise exception '0244 FAILED: an unbroken chain did not reduce to continuous (got %)', r.state;
    end if;
    ...
    begin
      ...
      values (b4,sid,probe_actor,'carr-internal','log-activity','deal',brk_subj,kb4,
              ...
              mb4,mb1);
    exception when others then
      raise exception '0244 FAILED: a STALE BUT REAL prior state was refused (%), '
    ...
    if r.state <> 'broken' then
      raise exception '0244 FAILED: a stale-but-real prior no longer produces a BROKEN chain (got %)', r.state;
    end if;
    if r.break_at is distinct from b4 then
      raise exception '0244 FAILED: the reducer did not name where the chain broke';
    end if;
    if r.conflict_count <> 0 then
      raise exception '0244 FAILED: a restatement that AGREES about material was counted as a conflict';
    end if;
    if r.head_digest is distinct from mb2 then
      raise exception '0244 FAILED: the reducer head is not the last MATERIAL claim';
    end if;
    ...
    exception when others then
      ...
      if position('never reached' in sqlerrm) = 0 then
        raise exception '0244 FAILED: a fabricated prior refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a receipt built on a state its subject never reached';
    end if;
    ...
    begin
      ...
      if ops.prove_write_receipt(junk) then
        raise exception '0244 FAILED: the bootstrap fixture proved';
      end if;
      failed := false;
      begin
        insert into ops.write_receipt (id, application_session_id, actor_id,
      ...
      exception when others then
        failed := true;
        if position('never reached' in sqlerrm) = 0 then
          raise exception '0244 FAILED: the bootstrap refused by the WRONG guard: %', sqlerrm;
        end if;
      end;
      if not failed then
        raise exception '0244 FAILED: an UNPROVEN receipt was usable as a prior state, '
  ...
  if ops.prove_write_receipt(bad) then
    raise exception '0244 FAILED: a receipt claiming a digest it never wrote was PROVEN';
  end if;
  ...
  exception when others then
    ...
    if position('is proven and cannot be retracted' in sqlerrm) = 0 then
      raise exception '0244 FAILED: retracting a proven receipt refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0244 FAILED: a PROVEN receipt was retracted';
  end if;
  ...
  exception when others then
    ...
    if position('same subject as the receipt it retracts' in sqlerrm) = 0 then
      raise exception '0244 FAILED: cross-subject retraction refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0244 FAILED: a retraction named a different subject than its target';
  end if;
  ...
  begin
    ...
    if ops.prove_write_receipt(foreign_receipt) then
      raise exception '0244 FAILED: the cross-tenant fixture proved when it should not';
    end if;
    ...
    exception when others then
      ...
      if position('cannot cross tenants' in sqlerrm) = 0 then
        raise exception '0244 FAILED: cross-tenant retraction refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: one tenant disavowed another tenant''s receipt';
    end if;
    ...
    exception when others then
      failed := true;
      if position('cannot cross tenants' in sqlerrm) = 0 then
        raise exception '0244 FAILED: cross-tenant reversal refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: one tenant reversed another tenant''s receipt';
    end if;
  end;
  ...
  if not ops.prove_write_receipt(ret) then
    raise exception '0244 FAILED: an honest retraction could not prove';
  end if;
  ...
  if (select count(*) from ops.receipt_conflicts('deal', ret_subj)) <> 0 then
    raise exception '0244 FAILED: an honest retraction manufactured a conflict '
  ...
  if r.receipt_count <> 0 or r.state <> 'empty' then
    raise exception '0244 FAILED: a retracted receipt and its retractor did not '
  ...
  begin
    ...
    if (select count(*) from ops.receipt_conflicts('deal', cnf_subj)) <> 1 then
      raise exception '0244 FAILED: two receipts disagreeing about material did not conflict';
    end if;
    ...
    begin
      ...
      if ops.prove_write_receipt(fake_rev) then
        raise exception '0244 FAILED: the unproven-reversal fixture proved';
      end if;
      if (select count(*) from ops.receipt_conflicts('deal', cnf_subj)) <> 1 then
        raise exception '0244 FAILED: an UNPROVEN reversal silenced a real conflict';
      end if;
      ...
      begin
        ...
        if (select count(*) from ops.receipt_conflicts('deal', cnf_subj)) <> 1 then
          raise exception '0244 FAILED: retracting a lying reversal laundered the conflict away';
        end if;
      end;
    end;
    ...
    begin
      ...
      if not ops.prove_write_receipt(real_rev) then
        raise exception '0244 FAILED: an honest reversal could not prove';
      end if;
      if (select count(*) from ops.receipt_conflicts('deal', cnf_subj)) <> 0 then
        raise exception '0244 FAILED: a PROVEN exact reversal did not close the conflict';
      end if;
    end;
  end;
  ...
  begin
    ...
    if (select count(*) from ops.receipt_conflicts('deal', shared)) <> 0 then
      raise exception '0244 FAILED: two TENANTS writing the same subject id were '
  ...
  begin
    ...
    if ops.write_receipt_material_digest(ks, sid, 'deal', s1) is distinct from before_other then
      raise exception '0244 FAILED: an unrelated subject''s event under the same call '
    ...
    if ops.write_receipt_material_digest(ks, sid, 'deal', s2) = before_other then
      raise exception '0244 FAILED: two different subjects under one call hashed the same';
    end if;
    ...
    if ops.write_receipt_material_digest(ks, sid3, 'deal', s1) = before_other then
      raise exception '0244 FAILED: the material digest ignores which session wrote the events';
    end if;
    ...
    if ops.write_receipt_material_digest(ka, sid, 'deal', s1)
      ...
      raise exception '0244 FAILED: the same two changes hashed differently depending on '
  ...
  exception when others then
    ...
    if position('write_receipt_reverses_xor_retracts' in sqlerrm) = 0 then
      raise exception '0244 FAILED: a reverse-and-retract receipt refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0244 FAILED: one receipt both reversed and retracted';
  end if;
    ...
    begin
      ...
      exception when others then
        ...
        if position('identity is immutable' in sqlerrm) = 0 then
          raise exception '0244 FAILED: rewriting material refused by the WRONG guard: %', sqlerrm;
        end if;
      end;
      if not failed then
        raise exception '0244 FAILED: a receipt''s MATERIAL CLAIM was rewritten in place';
      end if;
      ...
      exception when others then
        failed := true;
        if position('identity is immutable' in sqlerrm) = 0 then
          raise exception '0244 FAILED: rewriting the retraction link refused by the WRONG guard: %', sqlerrm;
        end if;
      end;
      if not failed then
        raise exception '0244 FAILED: a receipt was turned into a retraction after the fact';
      end if;
    end;
  ...
  exception when others then
    ...
    if position('does not exist' in sqlerrm) = 0 then
      raise exception '0244 FAILED: the five-argument recipe refused for the WRONG reason: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0244 FAILED: the subject-blind five-argument digest is still callable';
  end if;
    ...
    loop
      ...
      begin
        ...
        if not ops.prove_write_receipt(sweep_id) then
          raise exception '0244 FAILED: a sweep retraction of % failed to prove', r.id;
        end if;
      end;
    end loop;
  end;
    ...
    begin
      ...
      begin
        ...
        if ops.prove_write_receipt(u1) then
          raise exception '0244 FAILED: the three-level fixture proved when it should not';
        end if;
        insert into ops.write_receipt (id, application_session_id, actor_id,
          organization_tenant_id, verb, subject_type, subject_id,
          tool_call_idempotency_key, call_digest, material_digest, prior_digest,
        ...
        if ops.prove_write_receipt(ur) then
          raise exception '0244 FAILED: the unproven retraction proved';
        end if;
        insert into ops.write_receipt (id, application_session_id, actor_id,
          organization_tenant_id, verb, subject_type, subject_id,
          tool_call_idempotency_key, call_digest, material_digest, prior_digest,
        ...
        if not ops.prove_write_receipt(ur2) then
          raise exception '0244 FAILED: an honest second-level retraction could not prove';
        end if;
        failed := false;
        begin
        ...
        if not failed then
          raise exception '0244 FAILED: an UNPROVEN retraction cleared the acceptance bar';
        end if;
      end;
  ...
  begin
    ...
    exception when others then
      ...
      if position('does not name dependency' in sqlerrm) = 0 then
        raise exception '0244 FAILED: unrelated-receipt retirement refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a dependency was retired with receipts that never named it';
    end if;
    ...
    exception when others then
      ...
      if position('rest on the SAME call' in sqlerrm) = 0 then
        raise exception '0244 FAILED: same-call retirement refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a dependency was retired on two receipts about ONE call';
    end if;
    ...
    exception when others then
      ...
      if position('assert the SAME material state' in sqlerrm) = 0 then
        raise exception '0244 FAILED: same-material retirement refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a dependency was retired on two receipts asserting the same thing';
    end if;
    ...
    exception when others then
      ...
      if position('does not build on the repointed state' in sqlerrm) = 0 then
        raise exception '0244 FAILED: uncoupled-recovery retirement refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a recovery receipt that ignored the repoint was accepted';
    end if;
    ...
    if rdy.remaining <> 0 then
      raise exception '0244 FAILED: a retired dependency still counted as remaining';
    end if;
    ...
    if rdy.remaining <> 1 then
      raise exception '0244 FAILED: a WITHDRAWN retirement still counted as retired';
    end if;
    if rdy.ready then
      raise exception '0244 FAILED: readiness said yes with a withdrawn retirement';
    end if;
    ...
    exception when others then
      ...
      if position('cannot be rewritten' in sqlerrm) = 0 then
        raise exception '0244 FAILED: withdrawal rewrite refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0244 FAILED: a withdrawal was rewritten';
    end if;
    ...
    select * into rdy from ops.drive_retirement_readiness();
    if rdy.remaining <> 0 then
      raise exception '0244 FAILED: a dependency could not be retired again after '
    ...
    if rdy.retired_total <> 1 then
      raise exception '0244 FAILED: two live retirement rows for one dependency inflated '
