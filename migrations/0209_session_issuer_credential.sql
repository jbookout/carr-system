...
begin
  if not exists (select 1 from pg_roles where rolname='carr_session_issuer' and rolcanlogin) then
    raise exception '0209 FAILED: carr_session_issuer must exist and be able to log in; '
  ...
  if pg_has_role('carr_session_issuer', 'carr_writer', 'MEMBER')
    ...
    raise exception '0209 FAILED: carr_session_issuer can assume a write or '
  ...
  if members is distinct from 'carr_session_issuer' then
    raise exception '0209 FAILED: carr_session_minter members must be exactly '
