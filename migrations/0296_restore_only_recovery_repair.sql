-- 0296_restore_only_recovery_repair.sql
--
-- 0295's restore-only writer used a doubly escaped PostgreSQL regular
-- expression for a migration filename.  In a standard-conforming SQL string
-- that means "a literal backslash followed by any character", not the dot
-- before .sql.  A valid immutable candidate could therefore never prepare a
-- restore-only attempt.  0295 is already applied and immutable, so repair the
-- function forward rather than editing that evidence.
