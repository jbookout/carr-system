-- 0207 — mint a session from an actor SLUG, so the door needs no actor id
--
...
-- nothing but the mint. 0206 argues that the issuer must be unable to do
...
-- only thing the row is for: 0204's guard matches the evidence row's actor
...
begin
  ...
  -- Delegates rather than duplicating: every invariant 0204 enforces on the
...
declare
  offenders text;
begin
  -- Same sweep 0204 runs over its own functions, for the same reason: a grant
  ...
  if offenders is not null then
    raise exception '0207 FAILED: only carr_session_minter may mint by slug; found: %',
      offenders;
  end if;

  -- The delegation must actually reach 0204's function, or this migration has
  -- created a mint that enforces none of 0204's invariants.
  if not exists (
  ...
  then
    raise exception '0207 FAILED: the slug wrapper does not delegate to '
                    'ops.mint_application_session, so 0204''s invariants are bypassed';
  end if;
end $$;
