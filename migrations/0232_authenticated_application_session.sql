-- 0232_authenticated_application_session.sql
--
...
begin
  ...
  if offenders is not null then
    raise exception '0232 FAILED: column-level grants let these roles create or '
  ...
  select string_agg(format('%s:%s', grantee, privilege_type), ', ') into offenders
   ...
   where table_schema = 'ops' and table_name = 'application_session'
     ...
     and (grantee = 'PUBLIC'
          or not exists (select 1 from pg_roles where rolname = grantee and rolsuper));
  if offenders is not null then
    raise exception '0232 FAILED: these roles can create or mutate sessions, and '
  ...
  if offenders is not null then
    raise exception '0232 FAILED: only carr_session_minter may mint or revoke; found: %',
      offenders;
  end if;
  ...
  if offenders is not null then
    raise exception '0232 FAILED: these runtime roles can reach the mint, so the '
  ...
  if exists (select 1 from pg_roles where rolname='carr_session_minter' and rolcanlogin) then
    raise exception '0232 FAILED: carr_session_minter must be NOLOGIN';
  end if;
  ...
  if not exists (select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    ...
    raise exception '0232 FAILED: the mint must be SECURITY DEFINER';
  end if;
  ...
  if offenders is not null then
    raise exception '0232 FAILED: these functions do not pin search_path to exactly '
  ...
  select string_agg(p.proname, ', ') into offenders
   ...
   where n.nspname = 'ops'
     ...
     and regexp_replace(p.prosrc, '--[^\n]*', '', 'g') ~* (
         ...
         || '|execute\s+');
  if offenders is not null then
    raise exception '0232 FAILED: these guards reference something a guard has no '
  ...
  if (select count(*) from pg_constraint c
    ...
    raise exception '0232 FAILED: each audit table must carry a foreign key to '
  ...
  begin
    for i in 1 .. array_length(expected, 1) loop
      ...
      if not found then
        raise exception '0232 FAILED: trigger % is missing, or is not attached to % '
          'running %', expected[i][1], expected[i][2], expected[i][3];
      end if;
      if hit.tgenabled <> 'A' then
        raise exception '0232 FAILED: trigger % on % is not ENABLE ALWAYS',
          expected[i][1], expected[i][2];
      end if;
      if position('FOR EACH ROW' in upper(hit.def)) = 0 then
        raise exception '0232 FAILED: trigger % on % is not FOR EACH ROW',
          expected[i][1], expected[i][2];
      end if;
      if expected[i][3] = 'ops.refuse_application_session_relink' then
        ...
        if not (upper(hit.def) like '%WHEN ((OLD.APPLICATION_SESSION_ID IS DISTINCT FROM NEW.APPLICATION_SESSION_ID))%'
          ...
          raise exception '0232 FAILED: relink trigger % does not carry exactly the '
            'expected WHEN condition; it may be inert. Definition: %',
            expected[i][1], hit.def;
        end if;
      elsif hit.tgqual is not null then
        raise exception '0232 FAILED: trigger % on % carries a WHEN clause it must not '
      ...
      if position(expected[i][4] in upper(hit.def)) = 0 then
        raise exception '0232 FAILED: trigger % on % is the wrong shape (% absent). '
      ...
      if expected[i][5] <> '' and position(expected[i][5] in upper(hit.def)) > 0 then
        raise exception '0232 FAILED: trigger % on % must NOT cover %',
  ...
  if probe_actor is null or other_actor is null then
    raise exception '0232 FAILED: cannot prove the guards without two actor rows';
  end if;
  ...
  begin
    ...
    if not ops.application_session_is_live(probe_sid) then
      raise exception '0232 FAILED: a freshly minted session is not live';
    end if;
    ...
    if (select count(*) from public.tool_call where application_session_id = probe_sid) <> 1
      ...
      raise exception '0232 FAILED: a qualified insert reported success but wrote no row';
    end if;
    ...
    begin
      for k in 1 .. array_length(probes, 1) loop
        begin
          execute probes[k][3];
          raise exception '0232 FAILED: % was ACCEPTED but every guard here exists to '
            'refuse it. The plumbing may be present while the logic is not.', probes[k][2];
        exception
          when others then
            if sqlerrm like '0232 FAILED%' then raise; end if;
            if sqlerrm like 'PROBE_ROLLBACK%' then raise; end if;
            if position(lower(probes[k][1]) in lower(sqlerrm)) = 0 then
              raise exception '0232 FAILED: % was refused, but by a DIFFERENT guard than '
      ...
      if ran <> array_length(probes, 1) then
        raise exception '0232 FAILED: only % of % behaviour probes completed',
    ...
    if not exists (select 1 from public.tool_call
      ...
      raise exception '0232 FAILED: a legacy row could not be updated — the freeze '
    ...
    if exists (select 1 from public.tool_call where idempotency_key = legacy_key) then
      raise exception '0232 FAILED: a legacy row could not be removed — the freeze '
    ...
    if not ops.application_session_is_live(probe_sid) then
      raise exception '0232 FAILED: a live session reports not live';
    end if;
    if ops.application_session_is_live(expired_sid) then
      raise exception '0232 FAILED: an EXPIRED session reports live — the liveness '
    ...
    begin
      ...
      raise exception '0232 FAILED: revoking with an empty reason reported success';
    exception
      when others then
        if sqlerrm like '0232 FAILED%' then raise; end if;
        if position('requires a reason' in lower(sqlerrm)) = 0 then
          raise exception '0232 FAILED: the empty-reason refusal came from a '
    ...
    if ops.application_session_is_live(probe_sid) then
      raise exception '0232 FAILED: a revoked session still reports live';
    end if;
    begin
      insert into public.tool_call (idempotency_key, verb, actor_id, request_hash,
        response, organization_tenant_id, application_session_id)
      values ('p'||gen_random_uuid()::text,'probe',probe_actor,'h','{}'::jsonb,
              'carr-internal', probe_sid);
      raise exception '0232 FAILED: a REVOKED session still produced qualified evidence';
    exception
      when others then
        if sqlerrm like '0232 FAILED%' then raise; end if;
        if position('is revoked' in lower(sqlerrm)) = 0 then
          raise exception '0232 FAILED: the revoked-session refusal came from a '
            'different guard: %', sqlerrm;
        end if;
    end;
    ...
    begin
      for j in 1 .. array_length(after_revoke, 1) loop
        begin
          ...
          raise exception '0232 FAILED: % accepted evidence for a REVOKED session',
            after_revoke[j][1];
        exception
          when others then
            if sqlerrm like '0232 FAILED%' then raise; end if;
            if position('is revoked' in lower(sqlerrm)) = 0 then
              raise exception '0232 FAILED: % refused a revoked session by a different '
    ...
    begin
      ...
      raise exception '0232 FAILED: re-revoking reported success';
    exception
      when others then
        if sqlerrm like '0232 FAILED%' then raise; end if;
        if position('already revoked' in lower(sqlerrm)) = 0 then
          raise exception '0232 FAILED: the re-revocation refusal came from a '
            'different guard: %', sqlerrm;
        end if;
    end;
  ...
  exception
    when others then
      ...
      if sqlerrm <> sentinel then
        raise exception '0232 FAILED: the behaviour proof could not complete: %', sqlerrm;
      end if;
  end;
  ...
  if exists (select 1 from ops.application_session where id in (probe_sid, expired_sid))
    ...
    raise exception '0232 FAILED: the behaviour proof left rows behind';
  end if;
end $$;
