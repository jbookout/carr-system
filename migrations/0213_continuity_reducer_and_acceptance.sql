...
begin
  ...
  if probe_actor is null then
    raise exception '0213 FAILED: need a human actor to exercise acceptance';
  end if;

  -- empty subject reduces to empty
  select * into r from ops.continuity_reducer('deal', subject);
  if r.state <> 'empty' then
    raise exception '0213 FAILED: a subject with no receipts did not reduce to empty (got %)', r.state;
  end if;
  ...
  if r.state <> 'unproven' then
    raise exception '0213 FAILED: an unproven receipt did not reduce to unproven (got %)', r.state;
  end if;
  begin
    failed := false;
  ...
  if not failed then
    raise exception '0213 FAILED: Phase 4 was ACCEPTED while a receipt was unproven';
  end if;

  perform ops.prove_write_receipt(rid1);
  select * into r from ops.continuity_reducer('deal', subject);
  if r.state <> 'continuous' then
    raise exception '0213 FAILED: a single proven receipt did not reduce to continuous (got %)', r.state;
  end if;
  if r.head_digest is distinct from claimed then
    raise exception '0213 FAILED: the reducer head is not the last claimed digest';
  end if;
  ...
  if r.state <> 'broken' then
    raise exception '0213 FAILED: a chain with a gap did not reduce to broken (got %)', r.state;
  end if;
  if r.break_at is distinct from rid2 then
    raise exception '0213 FAILED: the reducer did not name where the chain broke';
  end if;
