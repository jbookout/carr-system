-- 0267 — write receipts: a session, a material digest, and a readback that is
...
-- and tenant — the same standard evidence rows are held to in 0264. There is no
...
-- digest itself. Those rows are frozen against update and delete by 0264 once
...
create table ops.write_receipt (
  ...
  -- The evidence row this receipt is a receipt FOR. Frozen by 0264 once bound.
...
-- ------------------------------------------------------- the session guard
-- Same standard as evidence in 0264: live, unexpired, unrevoked, and matching
...
begin
  ...
  -- proved against a legacy row would be a proof about something 0264 says
  -- proves nothing.
...
-- back. Same reasoning as 0264's block: this file runs where the contract suite
...
begin
  ...
  if probe_actor is null or other_actor is null then
    raise exception '0267 FAILED: need two seeded actors to exercise the guards';
  end if;
  ...
  exception when others then
    ...
    if position('different actor' in sqlerrm) = 0 then
      raise exception '0267 FAILED: cross-actor receipt refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0267 FAILED: a receipt naming a different actor than its session was ACCEPTED';
  end if;
  ...
  if (select is_proven from ops.write_receipt where id = rid) then
    raise exception '0267 FAILED: a receipt was proven before any readback ran';
  end if;

  proved := ops.prove_write_receipt(rid);
  if not proved then
    raise exception '0267 FAILED: an honest receipt did not prove against its frozen evidence';
  end if;
  if not (select is_proven from ops.write_receipt where id = rid) then
    raise exception '0267 FAILED: readback ran but is_proven did not follow it';
  end if;
  ...
  if ops.prove_write_receipt(rid2) then
    raise exception '0267 FAILED: a receipt claiming a digest it never wrote was PROVEN';
  end if;
  if (select is_proven from ops.write_receipt where id = rid2) then
    raise exception '0267 FAILED: a false claim still reported is_proven';
  end if;
  ...
  exception when others then
    ...
    if position('already carries a readback' in sqlerrm) = 0 then
      raise exception '0267 FAILED: re-proving refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0267 FAILED: a receipt was proven twice';
  end if;
  ...
  exception when others then
    ...
    if position('reversal is not exact' in sqlerrm) = 0 then
      raise exception '0267 FAILED: inexact reversal refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0267 FAILED: a reversal that does not restore the prior state was ACCEPTED';
  end if;
  ...
  if not failed then
    raise exception '0267 FAILED: a receipt digest was rewritten';
  end if;
  ...
  exception when others then
    ...
    if position('cannot be deleted' in sqlerrm) = 0 then
      raise exception '0267 FAILED: receipt deletion refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0267 FAILED: a receipt was deleted';
  end if;
  ...
  begin
    ...
    exception when others then
      ...
      if position('different session' in sqlerrm) = 0 then
        raise exception '0267 FAILED: cross-session readback refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0267 FAILED: a receipt was proven against another session''s evidence';
    end if;
  end;

  raise notice '0267 apply-time proof passed';
  raise exception 'ROLLBACK_0267_PROBE';
exception when others then
  if sqlerrm = 'ROLLBACK_0267_PROBE' then
    return;
  end if;
  raise;
end $$;
