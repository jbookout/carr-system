...
begin
  if (select mode from ops.rule_delivery_policy where singleton)
    ...
    raise exception '0349 REFUSED: rule-delivery map refresh requires shadow mode';
  end if;
  if (select count(*) from ops.rule_delivery_activation_target) <> 9 then
    raise exception '0349 REFUSED: activation target set is not exactly nine';
  end if;
  if exists (
    select 1 from ops.rule_delivery_activation_target
     where map_digest not in (v_old,v_new)
  ) then
    raise exception '0349 REFUSED: activation target carries an unknown map digest';
  end if;
  ...
  if v_updated not in (0,9) then
    raise exception '0349 REFUSED: partial activation target refresh changed % rows',v_updated;
  end if;
  if (select count(*) from ops.rule_delivery_activation_target where map_digest=v_new) <> 9 then
    raise exception '0349 FAILED: activation targets do not bind the exact current map';
  end if;
end $$;
