...
begin
  ...
  if probe_actor is null then
    raise exception '0243 FAILED: need a human actor for the retirement probe';
  end if;

  -- An empty inventory must NOT read as ready.
  select * into rdy from ops.drive_retirement_readiness();
  if rdy.ready then
    raise exception '0243 FAILED: an empty inventory reported READY; nothing proven '
  ...
  exception when others then
    ...
    if position('is not proven' in sqlerrm) = 0 then
      raise exception '0243 FAILED: unproven-receipt retirement refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0243 FAILED: a dependency was retired on UNPROVEN receipts';
  end if;
  ...
  exception when others then
    ...
    if position('drive_retirement_distinct_receipts' in sqlerrm) = 0 then
      raise exception '0243 FAILED: same-receipt retirement refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0243 FAILED: one receipt was accepted for BOTH the repoint and the recovery';
  end if;
  ...
  if rdy.remaining <> 0 then
    raise exception '0243 FAILED: a retired dependency still counted as remaining';
  end if;
  if rdy.ready then
    raise exception '0243 FAILED: retirement read as READY without an authority acceptance';
  end if;
  ...
  exception when others then
    ...
    if position('cannot be rewritten' in sqlerrm) = 0 then
      raise exception '0243 FAILED: retirement rewrite refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
