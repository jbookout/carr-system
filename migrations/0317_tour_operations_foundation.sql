...
begin
  if not exists (select 1 from ops.tour_public_projection p join ops.tour_field_assertion a on a.id=new.field_assertion_id and a.organization_tenant_id=new.organization_tenant_id join ops.tour_property_membership m on m.tour_id=p.tour_id and m.property_id=new.property_id and m.organization_tenant_id=new.organization_tenant_id and m.route_version=p.route_version where p.id=new.projection_id and p.organization_tenant_id=new.organization_tenant_id and new.route_version=p.route_version and a.property_id=new.property_id and a.field_key=new.display_field_key and a.review_state='reviewed' and a.data_classification='public') then raise exception 'projection fact lacks selected reviewed public assertion'; end if;
  return new;
end $$;
...
begin
  begin
    ...
    values ('10000000-0000-4000-8000-000000000002','tour-proof','proof','county',2,'sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb','https://example.invalid/terms',now(),'proof','tour', '["display.name"]','["source_intake","canonical_fact"]',now()-interval '1 hour','active','10000000-0000-4000-8000-000000000001');
    begin
    ...
    insert into ops.tour_public_projection (id,organization_tenant_id,tour_id,projection_version,route_version,facts_only,projection_digest,status) values ('10000000-0000-4000-8000-000000000060','tour-proof','10000000-0000-4000-8000-000000000040',1,1,true,'sha256:ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff','draft');
    ...
    exception when raise_exception then if sqlerrm <> 'projection fact lacks selected reviewed public assertion' then raise; end if; end;
