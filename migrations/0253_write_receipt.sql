...
begin
  ...
  if probe_actor is null or other_actor is null then
    raise exception '0241 FAILED: need two seeded actors to exercise the guards';
  end if;
  ...
  exception when others then
    ...
    if position('different actor' in sqlerrm) = 0 then
      raise exception '0241 FAILED: cross-actor receipt refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0241 FAILED: a receipt naming a different actor than its session was ACCEPTED';
  end if;
  ...
  if (select is_proven from ops.write_receipt where id = rid) then
    raise exception '0241 FAILED: a receipt was proven before any readback ran';
  end if;

  proved := ops.prove_write_receipt(rid);
  if not proved then
    raise exception '0241 FAILED: an honest receipt did not prove against its frozen evidence';
  end if;
  if not (select is_proven from ops.write_receipt where id = rid) then
    raise exception '0241 FAILED: readback ran but is_proven did not follow it';
  end if;
  ...
  if ops.prove_write_receipt(rid2) then
    raise exception '0241 FAILED: a receipt claiming a digest it never wrote was PROVEN';
  end if;
  if (select is_proven from ops.write_receipt where id = rid2) then
    raise exception '0241 FAILED: a false claim still reported is_proven';
  end if;
  ...
  exception when others then
    ...
    if position('already carries a readback' in sqlerrm) = 0 then
      raise exception '0241 FAILED: re-proving refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0241 FAILED: a receipt was proven twice';
  end if;
  ...
  exception when others then
    ...
    if position('reversal is not exact' in sqlerrm) = 0 then
      raise exception '0241 FAILED: inexact reversal refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0241 FAILED: a reversal that does not restore the prior state was ACCEPTED';
  end if;
  ...
  if not failed then
    raise exception '0241 FAILED: a receipt digest was rewritten';
  end if;
  ...
  exception when others then
    ...
    if position('cannot be deleted' in sqlerrm) = 0 then
      raise exception '0241 FAILED: receipt deletion refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0241 FAILED: a receipt was deleted';
  end if;
  ...
  begin
    ...
    exception when others then
      ...
      if position('different session' in sqlerrm) = 0 then
        raise exception '0241 FAILED: cross-session readback refused by the WRONG guard: %', sqlerrm;
      end if;
    end;
    if not failed then
      raise exception '0241 FAILED: a receipt was proven against another session''s evidence';
    end if;
  end;
