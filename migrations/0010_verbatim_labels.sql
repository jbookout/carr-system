-- 0010: verbatim-label columns from reconciliation round 2. The live files
-- carry free-text where the DB derives structure; exports must reproduce the
-- text verbatim, so both live side by side: the label (fidelity) and the
-- structured field (the future).
alter table lead   add column report_back_due_raw text;      -- 'On first contact'
alter table client add column deal_type_label text;          -- roster's own Deal Type text
alter table client add column possible_duplicate_label text; -- roster flag, verbatim
alter table vendor add column links_label text;              -- 'V-LEN-002; T-004' verbatim
