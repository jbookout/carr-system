-- 0269 — Drive retirement, resolved from receipts rather than from a report
--
...
-- because 0267 gave us receipts that prove themselves by readback and 0268 gave
...
-- two proven receipts. Declaring the phase closed is not, and lives in 0268's
...
begin
  ...
  if probe_actor is null then
    raise exception '0269 FAILED: need a human actor for the retirement probe';
  end if;

  -- An empty inventory must NOT read as ready.
  select * into rdy from ops.drive_retirement_readiness();
  if rdy.ready then
    raise exception '0269 FAILED: an empty inventory reported READY; nothing proven '
  ...
  begin
    ...
    values (gen_random_uuid(), sid, probe_actor, 'carr-internal',
            1, 1, 1, 1, 0, 0, '0269 empty-case probe');
    select * into rdy from ops.drive_retirement_readiness();
    if rdy.ready then
      raise exception '0269 FAILED: an empty inventory with an acceptance on record '
                      'reported READY';
    end if;
  ...
  exception when others then
    ...
    if position('is not proven' in sqlerrm) = 0 then
      raise exception '0269 FAILED: unproven-receipt retirement refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0269 FAILED: a dependency was retired on UNPROVEN receipts';
  end if;
  ...
  exception when others then
    ...
    if position('drive_retirement_distinct_receipts' in sqlerrm) = 0 then
      raise exception '0269 FAILED: same-receipt retirement refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0269 FAILED: one receipt was accepted for BOTH the repoint and the recovery';
  end if;
  ...
  if rdy.remaining <> 0 then
    raise exception '0269 FAILED: a retired dependency still counted as remaining';
  end if;
  if rdy.ready then
    raise exception '0269 FAILED: retirement read as READY without an authority acceptance';
  end if;
  ...
  insert into ops.phase4_acceptance
    (id, application_session_id, accepted_by_actor_id, organization_tenant_id,
     ...
     proven_receipts, unproven_receipts, open_conflicts, note)
  values (gen_random_uuid(), sid, probe_actor, 'carr-internal',
          1, 1, 1, 1, 0, 0, '0269 probe acceptance');
  select * into rdy from ops.drive_retirement_readiness();
  if not rdy.ready then
    raise exception '0269 FAILED: every dependency retired on proven receipts and an '
  ...
  exception when others then
    ...
    if position('cannot be rewritten' in sqlerrm) = 0 then
      raise exception '0269 FAILED: retirement rewrite refused by the WRONG guard: %', sqlerrm;
    end if;
  end;
  if not failed then
    raise exception '0269 FAILED: a retirement record was rewritten';
  end if;

  raise notice '0269 apply-time proof passed';
  raise exception 'ROLLBACK_0269_PROBE';
exception when others then
  if sqlerrm = 'ROLLBACK_0269_PROBE' then
    return;
  end if;
  raise;
end $$;
