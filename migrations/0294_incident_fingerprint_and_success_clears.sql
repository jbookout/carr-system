-- 0294_incident_fingerprint_and_success_clears.sql
...
insert into ops.incident (ref, title, severity, state, environment,
                          ...
                          detected_at)
values ('INC-0294-COLLIDE-1', 'collision proof (older)', 'SEV-3', 'detected',
        'local', 'proof', 'collector', 'proof',
        'migration-0294-collide|local|proof.job|exit_1', now() - interval '2 hours'),
       ('INC-0294-COLLIDE-2', 'collision proof (newer)', 'SEV-3', 'detected',
        'local', 'proof', 'collector', 'proof',
        'migration-0294-collide|local|proof.job|exit_2', now() - interval '1 hour');
...
select keep.id,
       'duplicate ' || dup.ref || ' folded in here by migration 0294: the same '
...
update ops.incident i
   set state = 'resolved',
       ...
       root_cause = 'duplicate of ' || keep.ref || ' — the same job failing the '
                    ...
                    || 'code. Folded by migration 0294; the evidence stays here '
...
begin
  select state, signature into v_keep_state, v_keep_sig
    from ops.incident where ref = 'INC-0294-COLLIDE-1';
  select state, root_cause into v_dup_state, v_dup_cause
    from ops.incident where ref = 'INC-0294-COLLIDE-2';

  if v_keep_state is distinct from 'detected' then
    raise exception '0294 FAILED: the merge closed the OLDER row (%), which is '
                    'the incident, not the duplicate', v_keep_state;
  end if;
  if v_keep_sig is distinct from 'migration-0294-collide|local|proof.job|exit_status' then
    raise exception '0294 FAILED: the surviving row did not take the normalized '
                    'fingerprint, reads %', v_keep_sig;
  end if;
  if v_dup_state is distinct from 'resolved' then
    raise exception '0294 FAILED: the duplicate row is still open (%)', v_dup_state;
  end if;
  ...
  if v_dup_cause is null or left(v_dup_cause, 12) <> 'duplicate of' then
    raise exception '0294 FAILED: a close-as-duplicate did not say so first: %',
                    coalesce(v_dup_cause, '(null)');
  end if;
  select count(*) into v_facts from ops.incident_fact f
    join ops.incident i on i.id = f.incident_id
   where i.ref = 'INC-0294-COLLIDE-1' and f.source_ref = 'migration:0294';
  if v_facts <> 1 then
    raise exception '0294 FAILED: the survivor should carry exactly one fact '
                    'pointing at the folded duplicate, has %', v_facts;
  end if;

  delete from ops.incident where ref like 'INC-0294-COLLIDE-%';
  raise notice '0294: a collision folds into the older row, which keeps the '
...
comment on column ops.incident.signature is
  ...
  'index). Since 0294 the failure class is normalized before it lands here: a '
...
begin
  insert into ops.service (key, name, family, criticality, owner_actor)
    values ('migration-0294-proof', 'proof', 'Data', 'medium', 'system')
    returning id into v_service;

  -- 1. AN INCIDENT WITH NO SUCCESS SEQUENCE DOES NOT CLOSE.
  insert into ops.incident (ref, title, severity, state, environment,
                            detected_source, source_kind, source_ref, signature)
    values ('INC-0294-PROOF-1', 'proof', 'SEV-3', 'detected', 'local',
            'proof', 'collector', 'proof',
            'migration-0294-proof|local|proof.job|exit_status')
    returning id into v_inc;
  select ops.clear_recovered_incident('INC-0294-PROOF-1', 3) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: closed an incident with no green runs at all';
  end if;
  ...
  for i in 1..2 loop
    ...
    insert into ops.run (kind, service_id, environment, run_key, state,
      ...
      values ('job', v_service, 'local', 'proof.job', 'succeeded',
              v_t, v_t, v_t, 'operator', 'migration-0294-proof');
  end loop;
  select ops.clear_recovered_incident('INC-0294-PROOF-1', 3) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: two healthy runs satisfied a three-run sequence';
  end if;
  ...
  select ops.clear_recovered_incident('INC-0294-PROOF-1', 3) into v_ok;
  if not v_ok then
    raise exception '0294 FAILED: three consecutive healthy runs did not close it';
  end if;
  if not exists (select 1 from ops.incident
                  where ref = 'INC-0294-PROOF-1' and state = 'resolved'
                    and resolved_at is not null
                    and recovery_evidence_ref = 'ops.run:' || v_run::text) then
    raise exception '0294 FAILED: the close did not carry the green run as evidence';
  end if;
  ...
  insert into ops.incident (ref, title, severity, state, environment,
                            detected_source, source_kind, source_ref, signature)
    values ('INC-0294-PROOF-2', 'proof', 'SEV-3', 'detected', 'local',
            'proof', 'collector', 'proof',
            'migration-0294-proof|local|proof.flaky|exit_status');
  v_t := now() - interval '1 hour';
  for i in 1..2 loop
    v_t := v_t + interval '1 minute';
    insert into ops.run (kind, service_id, environment, run_key, state,
  ...
  insert into ops.run (kind, service_id, environment, run_key, state,
    ...
    values ('job', v_service, 'local', 'proof.flaky', 'succeeded',
            v_t, v_t, v_t, 'operator', 'migration-0294-proof');
  select ops.clear_recovered_incident('INC-0294-PROOF-2', 3) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: a failure inside the window did not break the sequence';
  end if;
  ...
  select ops.clear_recovered_incident('INC-0294-PROOF-2', 3) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: a skipped run completed a success sequence';
  end if;
  v_t := v_t + interval '1 minute';
  insert into ops.run (kind, service_id, environment, run_key, state,
                       started_at, ended_at, observed_at, source_kind, source_ref)
  ...
  select ops.clear_recovered_incident('INC-0294-PROOF-2', 3) into v_ok;
  if not v_ok then
    raise exception '0294 FAILED: three green runs either side of a skip did not close it';
  end if;

  -- 5. SEV-1 NEVER CLOSES HERE, however perfect the evidence.
  insert into ops.incident (ref, title, severity, state, environment,
                            detected_source, source_kind, source_ref, signature)
    values ('INC-0294-PROOF-3', 'proof', 'SEV-1', 'detected', 'local',
            'proof', 'collector', 'proof',
            'migration-0294-proof|local|proof.job|dependency_unavailable');
  select ops.clear_recovered_incident('INC-0294-PROOF-3', 3) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: a SEV-1 closed without a human';
  end if;

  -- 6. A HAND-OPENED INCIDENT NEVER CLOSES HERE EITHER.
  insert into ops.incident (ref, title, severity, state, environment,
                            detected_source, source_kind, source_ref, signature)
    values ('INC-0294-PROOF-4', 'proof', 'SEV-3', 'detected', 'local',
            'proof', 'operator', 'proof',
            'migration-0294-proof|local|proof.job|hand_written');
  select ops.clear_recovered_incident('INC-0294-PROOF-4', 3) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: the machine closed an incident a human opened';
  end if;
  ...
  insert into ops.incident (ref, title, severity, state, environment,
                            detected_source, source_kind, source_ref, signature)
    values ('INC-0294-PROOF-5', 'proof', 'SEV-3', 'detected', 'local',
            'proof', 'collector', 'proof',
            'migration-0294-proof|local|proof.once|exit_status');
  insert into ops.run (kind, service_id, environment, run_key, state,
                       started_at, ended_at, observed_at, source_kind, source_ref)
    values ('job', v_service, 'local', 'proof.once', 'succeeded',
            now(), now(), now(), 'operator', 'migration-0294-proof');
  select ops.clear_recovered_incident('INC-0294-PROOF-5', 1) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: a caller asking for one run got one run';
  end if;
  select ops.clear_recovered_incident('INC-0294-PROOF-5', 0) into v_ok;
  if v_ok then
    raise exception '0294 FAILED: a caller asking for zero runs got zero runs';
  end if;
  ...
  insert into ops.incident (ref, title, severity, state, environment,
                            detected_source, source_kind, source_ref, signature)
    values ('INC-0294-PROOF-6', 'proof', 'SEV-3', 'detected', 'local',
            'proof', 'collector', 'proof',
            'migration-0294-proof|local|restore.key-recovery|pubkey_mismatch'),
           ('INC-0294-PROOF-7', 'proof', 'SEV-3', 'detected', 'local',
            'proof', 'collector', 'proof',
            'migration-0294-proof|local|restore.key-recovery|restore_failed');

  delete from ops.incident where ref like 'INC-0294-PROOF-%';
  delete from ops.run where service_id = v_service;
  delete from ops.service where id = v_service;

  raise notice '0294: three consecutive healthy runs close a SEV-3 job incident with '
