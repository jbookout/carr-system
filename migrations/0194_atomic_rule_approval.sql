...
revoke all on ops.enforcement_control_catalog,ops.rule_control_binding,ops.rule_approval_receipt,
  ops.rule_retirement_receipt,ops.v_rule_enforcement_status from public;
...
begin
  ...
  if has_function_privilege('carr_writer',
    ...
    raise exception '0194 FAILED: routine writer may install semantic rule bindings';
  end if;
  if has_function_privilege('carr_writer',
       'ops.retire_rule(uuid,text,uuid,text)'::regprocedure,'execute') then
    raise exception '0194 FAILED: routine writer may retire rules';
  end if;
