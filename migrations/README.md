...
`0232_authenticated_application_session.sql` is the authenticated application-session
...
`0232` creates `carr_session_minter`, and that role is the whole of its separation
argument: on the day the snapshot's ledger passes 0232, the migration stops replaying
