...
`0257_authenticated_application_session.sql` is the authenticated application-session
...
`0257` creates `carr_session_minter`, and that role is the whole of its separation
argument: on the day the snapshot's ledger passes 0257, the migration stops replaying
