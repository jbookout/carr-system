-- 0225_atomic_rule_lifecycle_forward_upgrade.sql
...
-- the semantic binding sync; a stale/missing snapshot must not make 0225 fail
...
begin
  if (select count(*) from ops.enforcement_control_catalog where
    ...
    raise exception '0225 FAILED: control catalog does not match the two exact reviewed controls';
  end if;
end $$;
...
begin
  if to_regprocedure('ops.approve_rule(uuid,text,text[],text,text)') is null then
    raise exception '0225 FAILED: enforced approval function is missing';
  end if;
  if has_function_privilege('carr_writer',
       'ops.approve_rule(uuid,text,text[],text,text)'::regprocedure,'execute') then
    raise exception '0225 FAILED: routine writer may approve rules';
  end if;
  if has_function_privilege('carr_writer',
       'ops.sync_system_rule_control_bindings()'::regprocedure,'execute') then
    raise exception '0225 FAILED: routine writer may install semantic rule bindings';
  end if;
  if has_function_privilege('carr_writer',
       'ops.retire_rule(uuid,text,uuid,text)'::regprocedure,'execute') then
    raise exception '0225 FAILED: routine writer may retire rules';
  end if;
  if exists (select 1 from ops.enforcement_control_catalog
              where control_key='standing_context_runtime') then
    raise exception '0225 FAILED: prose surfacing is mislabeled as unbreakable enforcement';
  end if;
  if not exists (select 1 from ops.enforcement_control_catalog
    ...
    raise exception '0225 FAILED: platform metering control is not registered';
  end if;
end $$;
