-- 0228_atomic_rule_lifecycle_forward_upgrade.sql
...
-- the semantic binding sync; a stale/missing snapshot must not make 0228 fail
...
begin
  if (select count(*) from ops.enforcement_control_catalog where
    ...
    raise exception '0228 FAILED: control catalog does not match the two exact reviewed controls';
  end if;
end $$;
...
create table ops.rule_retirement_receipt (
  ...
  rule_version_after    integer not null check (rule_version_after=rule_version_before+1),
...
-- 0194 receipts are append-only and recorded the pre-activation version.
-- Preserve those rows verbatim, but accept them only through an append-only
-- migration anchor after proving the post-activation rule and every covered
-- contract/control/authority fact still match. Anything that fails that proof
-- stays absent from the compiler and cannot replay as a current approval.
create table ops.rule_approval_lifecycle_anchor (
  approval_receipt_id uuid primary key references ops.rule_approval_receipt(id) on delete restrict,
  rule_id uuid not null references rule(id) on delete restrict,
  rule_version_after integer not null check (rule_version_after>0),
  statement_hash text not null check (statement_hash ~ '^[0-9a-f]{64}$'),
  anchored_at timestamptz not null default now(),
  unique (rule_id,rule_version_after)
);
...
create trigger rule_approval_lifecycle_anchor_valid
  before insert on ops.rule_approval_lifecycle_anchor
  for each row execute function ops.require_rule_approval_lifecycle_anchor();

create trigger rule_approval_lifecycle_anchor_append_only
  before update or delete on ops.rule_approval_lifecycle_anchor
  for each row execute function ops.refuse_rule_approval_receipt_rewrite();

insert into ops.rule_approval_lifecycle_anchor
  (approval_receipt_id,rule_id,rule_version_after,statement_hash)
select ar.id,r.id,r.version,ar.statement_hash
  from ops.rule_approval_receipt ar
  join rule r on r.id=ar.rule_id and r.status='active'
  join actor joe on joe.id=ar.actor_id
   and joe.slug='joe' and joe.kind='human' and joe.active
  join ops.rule_admission a on a.rule_id=r.id
 where ar.rule_version+1=r.version
   and ar.actor_id=r.activated_by
   and ar.statement_hash=encode(digest(r.statement,'sha256'),'hex')
   and r.enforcement=(case when ar.enforcement_status='hard_enforced'
                           then 'gate' else 'constraint' end)
   and a.state='admitted' and a.admitted_by=ar.actor_id
   ...
   and not exists (
     select 1 from unnest(ar.requested_control_keys) requested(control_key)
      where not exists (
        select 1 from ops.rule_enforcement_point ep
        join ops.enforcement_control_catalog c using (control_key)
        join ops.rule_control_binding b on b.rule_id=ep.rule_id and b.control_key=ep.control_key
         where ep.rule_id=r.id and ep.control_key=requested.control_key
           and ep.installed and c.installed and c.verified_at is not null
           and b.statement_hash=ar.statement_hash
           and c.enforcement_class in ('deny_gate','stop_gate','schema','transactional_schema')))
on conflict (approval_receipt_id) do nothing;

revoke all on ops.rule_approval_lifecycle_anchor from public;
...
revoke all on ops.enforcement_control_catalog,ops.rule_control_binding,ops.rule_approval_receipt,
  ops.rule_approval_lifecycle_anchor,ops.rule_retirement_receipt,ops.v_rule_enforcement_status from public;
...
begin
  if to_regprocedure('ops.approve_rule(uuid,text,text[],text,text)') is null then
    raise exception '0228 FAILED: enforced approval function is missing';
  end if;
  if has_function_privilege('carr_writer',
       'ops.approve_rule(uuid,text,text[],text,text)'::regprocedure,'execute') then
    raise exception '0228 FAILED: routine writer may approve rules';
  end if;
  if has_function_privilege('carr_writer',
       'ops.sync_system_rule_control_bindings()'::regprocedure,'execute') then
    raise exception '0228 FAILED: routine writer may install semantic rule bindings';
  end if;
  if has_function_privilege('carr_writer',
       'ops.retire_rule(uuid,text,uuid,text)'::regprocedure,'execute') then
    raise exception '0228 FAILED: routine writer may retire rules';
  end if;
  if exists (select 1 from ops.enforcement_control_catalog
              where control_key='standing_context_runtime') then
    raise exception '0228 FAILED: prose surfacing is mislabeled as unbreakable enforcement';
  end if;
  if not exists (select 1 from ops.enforcement_control_catalog
    ...
    raise exception '0228 FAILED: platform metering control is not registered';
  end if;
end $$;
