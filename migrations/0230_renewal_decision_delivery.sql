-- 0230_renewal_decision_delivery.sql — safe, record-native renewal delivery.
--
...
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='v_renewal_decision_queue' and column_name=any(forbidden)) then
    raise exception '0230 FAILED: renewal decision queue exposes a prohibited raw field';
  end if;
  if not has_table_privilege('carr_reader','v_renewal_decision_queue','select')
    ...
    raise exception '0230 FAILED: renewal reader grant boundary is wrong';
  end if;
  if has_table_privilege('carr_reader','ops.renewal_decision_source_run','select')
    ...
    raise exception '0230 FAILED: renewal source-run base tables leaked';
  end if;
end $$;
