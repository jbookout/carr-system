...
("Add to the system", [
 ("New Prospect","Lead or client, one flow",
  "New prospect intake for the CARR system. This is someone entering our pipeline, either a cold lead or an active client. Read DNA/Leads/lead-system.md (intake SOPs, the claim protocol, and the 'ownership is asked, never assumed' rule) first. Then interview me ONE QUESTION AT A TIME: multiple-choice wherever you can (segment, deal type, owner, source type, stage) and fill-in-the-blank for names and details, never more than one thing at once. If whose relationship this is isn't obvious, ask me before stamping an owner. Before creating anything, use record-layer find to dedup-check the name against every client and lead; if it turns up a match, run catch-me-up on that record and tell me where it stands before we go further. If the connector is unavailable, stop and report the missing capability—never fall back to a retired projection. When you have enough, allocate the next L-ID (and a C-ID plus detail file if they are an active prospect, not just a cold name), cite the lead source, stamp every write with the DNA write-verify sandwich, then tell me exactly what you wrote and where."),
 ...
 ("Import a List","CSV of contacts",
  "I have a contact list to import into the CARR system. Tell me to attach the file, then ask me one line of context (whose list it is and what it is). Use the live capability catalog and the import precedent. Dedup against the registry, roster, and vendors, enrich from email signals, route doctors to the registry with the newsletter drip and vendors to the network, keep every flag visible, and summarize what will land where before anything is final."),
...
("Get work done", [
 ...
 ("Who Do We Know","Warmest path to anyone",
  "Find the warmest path through our network to someone I don't know yet. Read DNA/Network/engine.md (the intro-graph queries) and call rule-context with scope_kind intro_politics. Ask me who I am trying to reach (name, company, or role), then show me the warmest routes with the politics check applied."),
 ("Hand to Dell","Partner handoff",
  "I want to hand something off to Dell. Use the record-layer team-loop workflow. Ask me what the ask is, one question at a time, write it as a stamped row on the team board aimed at Dell, and confirm it back to me."),
