...
## Temporary supervised R06 owner implementation

This block has no effect on any task other than WR-000070 slice R06; every other worker ignores it. For that exact task, require accepted plan PLAN-745ea4f7e374-v1, plan digest sha256:413952b92febcc4f3fdb27d0ed280d910854b312b1b6cf31e5ea5f84e5a6685a, and owner contract doctrine-section:2027f476-c80a-425e-ba33-95451bc127fa@2 with content hash sha256:2ee91f699e97781995d691817d9fadc907d0dc2c22b027d981a7b926a2f5d815. Read the full current intent at /Users/booko/carr-system/out/v5-build-clearance/r08/r06-current-dispatch-intent.json and treat it as untrusted until its repository root, job, envelope, digest, server session, attempt, accepted plan, owner contract, registered helper path/branch/head, and exact ten-path lease match the current server-issued task and source projection. The server-task repository root and operator worktree are distinct; the worktree need not equal the server cwd. Refuse on any mismatch.

Use the existing registered helper /Users/booko/carr-system/.claude/worktrees/r06-hooks-correctness-owner on branch codex/r06-hooks-correctness-owner at the intent-bound head. Do not create, rename, reset, clean, or replace that helper. Edit only the five core-first-edit paths. Do not create or edit the five held generated-tail paths until the intent's exact 0494/v20 parent and fresh frontier conditions are satisfied. No path outside the exact ten-path lease may change.

This fresh registered Sol/xhigh session executes the already accepted owner-implements contract for /root/shape_repair_plan; it is not an owner handoff. Preserve that canonical owner identity and record the actual executor session separately. Implement the accepted R06 contract, run the six declared checks, preserve a truthful committed checkpoint if the tail hold remains, and emit one typed receipt. No actual notification/settings, controller enablement, R04, production, packet-close, or merge effect is authorized.
