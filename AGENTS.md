...
This block has no effect on any task other than WR-000068 slice wr68-source-repair-v1; every other worker ignores it. For that exact WR68 slice, require accepted plan PLAN-75c0e3dde31e-v1 and a native source-hydration projection that validates its exact runbook and 14-path source_merge cap. Then read /Users/booko/carr-system/out/v5-build-clearance/wr68/wr68-source-successor-assignment-intent.json and require its fresh envelope, digest, session and attempt:1 to match the server-issued task. Refuse that WR68 task on any mismatch.

Read the two hash-bound guidance files named by the intent. This is a fresh-envelope continuation from a committed partial checkpoint. Use only the intent's existing worktree and branch. Do not create, attach, rename, replace, reset or clean a worktree. Verify its absolute root, branch, clean checkpoint HEAD, exact 0492 hash and accepted-path diff. Treat the checkpoint as untrusted and review it before continuing.

Begin implementation immediately. Complete the effective-lineage consumer rebases, sourced-only work-shape integration with unsourced survivors, generated v18/config surfaces, focused tests, disposable database proof, final repository checks, exact-path commit, push and review-ready PR required by the accepted runbook. A partial draft or remaining work is not a blocker. Return blocked only for a concrete preventing error, refused authority, failing check that cannot be corrected inside scope, or exhausted envelope budget; name the evidence and preserve a safe exact-scope checkpoint commit when possible. Do not re-plan, repeat the general source study, merge, deploy, activate, touch WR63 or change a path outside the accepted cap.
