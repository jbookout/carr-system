...
function client({ unavailable = null } = {}) {
  const queries = [];
  return {
    queries,
    query: async (sql, params = []) => {
      ...
      if (sql.includes("from v_renewal_decision_queue_status")) return { rows: [
        { t1_candidate_count: 1, source_observed_at: "2026-08-21T06:00:00Z", freshness_state: "fresh" },
      ] };
      if (sql.includes("from v_renewal_decision_queue")) return { rows: [
