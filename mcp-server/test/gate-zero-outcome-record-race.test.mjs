...
test("an outer failure after the seat commit leaves no event, and the retry writes exactly one",
  async t => {
    ...
    // (3) THE RETRY. A new request -- its own correlation id, its own
    //     idempotency key, its own transactions -- reaches the same candidate.
    //     Before 0505 the record layer refused any retry whose projection had
    //     moved, and the gateway refused again over its own copy of that
    //     comparison; the row was reachable and the event was not.
