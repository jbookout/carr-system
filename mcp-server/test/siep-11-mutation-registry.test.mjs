...
test("the ACTIVE runtime registry is v63, and a stale v19 import fails admission", async () => {
  ...
  // v65 registers the resource-observation pair; v69 registers
  // record-workflow-census and read-workflow-census after v66-v68 registered none.
  ...
  assert.equal(SCAC_MUTATION_REGISTRY_DIGEST, v69SelectorDigest);
...
test("the v21 frontier re-digested only source, and v63 is what the runtime now imports", async () => {
  ...
  const v63GeneratedDigest = generatedV63.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1];
  assert.equal(SCAC_MUTATION_REGISTRY_DIGEST, generatedV69.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1]);
  assert.notEqual(SCAC_MUTATION_REGISTRY_DIGEST, generatedV65.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1]);
  ...
  const liveRows = frozenInventory(REGISTRY_V69_VERSION);
...
test("v33 seals the notification preference pair and preserves v32", () => {
  ...
  // The complete generated frontier now includes the v69 successor;
  // 0527 remains handwritten and does not move that count.
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("migrations/")).length, 75);
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("mcp-server/src/")).length, 66);
});
...
test("the complete source-only frontier is byte-reproducible from frozen inputs", () => {
  assert.equal(assertCurrentSourceInventoryMatchesFixture(TOOLS, REGISTRY_V69_VERSION), true);
  ...
  assert.equal(paths.filter(path => path.endsWith(".generated.js")).length, 66);
...
test("GitHub and launchd workflow entrances bind exact triggers, permissions, and delegates", () => {
  ...
  // com.carr.workflow-census-writer (the V5-F09 census writer) is the 39th.
