...
test("reviewed MCP inventory is an exact immutable projection of the assembled registry", () => {
  ...
  // DoctorCRE V5-J102 (0630) adds twenty writes and one read: the CRE
  // lifecycle door's twenty-one verbs.
  assert.equal(rows.length, 330);
  assert.equal(rows.filter(row => row.write).length, 239);
...
test("the ACTIVE runtime registry is v63, and a stale v19 import fails admission", async () => {
  ...
  // record-source-authority verbs; v78 registers the twenty-one V5-J102
  // CRE lifecycle verbs.
...
test("v33 seals the notification preference pair and preserves v32", () => {
  ...
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("migrations/")).length, 84);
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("mcp-server/src/")).length, 75);
});
