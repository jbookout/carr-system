...
test("v33 seals the notification preference pair and preserves v32", () => {
  ...
  // The complete generated frontier includes every delta-sealed successor;
...
test("reviewed non-MCP source locators resolve and remain explicitly non-authorizing", () => {
  ...
  // v57 (the delta seal, 2026-09-23) adds ops/scac-seal-successor.py, the
  ...
  // v57 adds the delta-seal driver ops/scac-seal-successor.py.
...
test("delta seal: the generic renderer reproduces the hand-rendered v55 and v56 migrations byte for byte", () => {
  const committed = path => fs.readFileSync(new URL(`../../${path}`, import.meta.url), "utf8");
  for (const ordinal of [55, 56]) {
...
test("delta seal: the successor config is validated as a contiguous chain from v55", async () => {
  ...
  const cases = [
    ...
    ["a migration number that skips", config => { config.successors[1].migration = "migrations/0565_ci_split_scac_successor.sql"; }, /migration must be/],
