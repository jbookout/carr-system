...
test("reviewed MCP inventory is an exact immutable projection of the assembled registry", () => {
  ...
  // V5-A05 delivery cadence (0610) adds three: cadence-status (read),
  // record-cadence-receipt and raise-delivery-cadence-alert (writes).
...
test("the ACTIVE runtime registry is v63, and a stale v19 import fails admission", async () => {
  ...
  assert.equal(SCAC_MUTATION_REGISTRY_VERSION, REGISTRY_V74_VERSION);
  const v74SelectorDigest = generatedV74.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1];
  assert.equal(SCAC_MUTATION_REGISTRY_DIGEST, v74SelectorDigest);
...
test("the v21 frontier re-digested only source, and v63 is what the runtime now imports", async () => {
  ...
  const v63GeneratedDigest = generatedV63.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1];
  assert.equal(SCAC_MUTATION_REGISTRY_DIGEST, generatedV74.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1]);
  ...
  const liveRows = frozenInventory(REGISTRY_V74_VERSION);
...
test("v33 seals the notification preference pair and preserves v32", () => {
  ...
  // The complete generated frontier now includes the v74 successor;
  // 0527 remains handwritten and does not move that count.
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("migrations/")).length, 80);
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("mcp-server/src/")).length, 71);
});
...
test("the complete source-only frontier is byte-reproducible from frozen inputs", () => {
  assert.equal(assertCurrentSourceInventoryMatchesFixture(TOOLS, REGISTRY_V74_VERSION), true);
...
test("GitHub and launchd workflow entrances bind exact triggers, permissions, and delegates", () => {
  ...
  // 40, not 39: a launchd plist landed on main since #1241's
  // com.carr.nightly-exports-daytime-retry.plist, a real deployed launchd
  // entrance unrelated to the SCAC seal chain itself.
  ...
  // 33, not 32 — the same main-branch plist addition noted above.
  ...
  // 32, not 31 — the same main-branch plist addition noted above: one more
  // deployed plist, one more ops.service_environment: ref.
