...
test("reviewed MCP inventory is an exact immutable projection of the assembled registry", () => {
  ...
  // V5-RW02 adds three durable evidence writers and one per-action evidence read.
  assert.equal(rows.length, 348);
  assert.equal(rows.filter(row => row.write).length, 252);
...
test("the ACTIVE runtime registry is v63, and a stale v19 import fails admission", async () => {
  ...
  // V5-RW02's four durable evidence-store operations without provider authority.
