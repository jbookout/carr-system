...
test("reviewed MCP inventory is an exact immutable projection of the assembled registry", () => {
  ...
  assert.equal(rows.length, 363);
  assert.equal(rows.filter(row => row.write).length, 261);
  assert.equal(rows.filter(row => !row.write).length, 102);
...
test("the ACTIVE runtime registry is v63, and a stale v19 import fails admission", async () => {
  ...
  // without provider authority.
  assert.equal(SCAC_MUTATION_REGISTRY_VERSION, REGISTRY_V88_VERSION);
  const v88SelectorDigest = generatedV88.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1];
  assert.equal(SCAC_MUTATION_REGISTRY_DIGEST, v88SelectorDigest);
...
test("the v21 frontier re-digested only source, and v63 is what the runtime now imports", async () => {
  ...
  const v63GeneratedDigest = generatedV63.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1];
  assert.equal(SCAC_MUTATION_REGISTRY_DIGEST, generatedV88.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1]);
  assert.notEqual(SCAC_MUTATION_REGISTRY_DIGEST, generatedV87.match(
    /^export const SCAC_MUTATION_REGISTRY_DIGEST = "([0-9a-f]{64})";$/m)[1]);
  assert.notEqual(SCAC_MUTATION_REGISTRY_DIGEST, generatedV85.match(
  ...
  const liveRows = frozenInventory(REGISTRY_V88_VERSION);
...
test("v33 seals the notification preference pair and preserves v32", () => {
  ...
  // The complete generated frontier now includes the provisional v88 successor;
  // 0527 remains handwritten and does not move that count.
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("migrations/")).length, 94);
  assert.equal(Object.keys(renderGeneratedFrontier())
    .filter(path => path.startsWith("mcp-server/src/")).length, 85);
});
...
test("the complete source-only frontier is byte-reproducible from frozen inputs", () => {
  assert.equal(assertCurrentSourceInventoryMatchesFixture(TOOLS, REGISTRY_V88_VERSION), true);
