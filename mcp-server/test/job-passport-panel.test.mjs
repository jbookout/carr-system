...
const portfolio = JSON.parse(fs.readFileSync(new URL("../../control-room/contracts/fixtures/execution-fabric/job-passport-eval-portfolio.synthetic.v1.json", import.meta.url)));
...
test("typed facts count on the wire but cannot create a visual job from transcript-like data", () => {
  ...
  assert.deepEqual(model.typedCounts, { execution_envelope: 1, progress_event: 1, attempt_receipt: 1, observatory_projection: 0, eval_portfolio: 0 });
});

test("eval ladder binds to the exact visual projection and keeps critical regression visible without a score", () => {
  const model = deriveJobPassports([
    turn(wrap("observatory_projection", fixture), 1), turn(wrap("eval_portfolio", portfolio), 2),
  ]);
  assert.equal(model.passports[0].eval_portfolio.portfolio_id, "portfolio:job-passport-synthetic");
  ...
  const withheld = deriveJobPassports([turn(wrap("observatory_projection", fixture), 1), turn(wrap("eval_portfolio", mismatched), 2)]);
