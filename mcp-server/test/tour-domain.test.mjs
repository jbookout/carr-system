...
test("writes use all exact revised SQL seams, derived tenancy, and never leak internal request or cheat-sheet content", async () => {
  ...
  for(const event of h.events)assert.doesNotMatch(JSON.stringify(event),/private_notes|access_notes|routing_request/); assert.equal(h.envelopes.length,7);
});
