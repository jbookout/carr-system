...
function mockDatabase({ existing = null, seatConnection = true } = {}) {
  ...
  const db = {
    ...
    query: async (sql, params = []) => {
      ...
      // THE EVENT GUARD'S TWO STATEMENTS. The lock is always taken here — a
      // single-threaded mock has no second transaction to lose it to — and the
      // existence read answers from what this mock has actually been asked to
      // insert, so "one event per outcome row" is a property this mock can
      // exhibit rather than one it asserts.
      if (sql.includes("pg_try_advisory_xact_lock")) return { rows: [{ taken: true }] };
