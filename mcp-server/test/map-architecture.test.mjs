...
test("map-architecture returns the two live sources and machine contract", async () => {
  ...
  assert.deepEqual(result.sources.map(item => item.version), [1, 2]);
  assert.equal(result.contract.path, "workspace/contracts/market-map-route-planning.v1.json");
  assert.equal(result.contract.version, "1.1.0");
  assert.match(result.required_workflow.join(" "), /canonical.*GIS.*routing.*navigation/i);
});
