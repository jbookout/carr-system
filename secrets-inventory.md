...
| GOOGLE_CLIENT_ID | n/a (auth layer) | upstream Google sign-in for the connector; scopes `openid email` only | Worker secret (wrangler) | JOE creates the OAuth client and runs `wrangler secret put` himself — Claude never holds the value. Redirect URI is exactly `https://api.practicecre.com/callback` |
...
| PARTNER_TOKENS | WRITE (via the verbs, as joe or dell) | INTERIM per-partner bearer for /mcp | Worker secret (wrangler) | RETIREMENT IS A LISTED STEP, not a side effect: delete once BOTH partners' OAuth connectors verify live on their phones. Until then /mcp accepts it and an OAuth token both |
