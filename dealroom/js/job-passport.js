...
function validExecutionEnvelope(value, passport) {
  ...
  if (!exactKeys(value, ["schema_version", "envelope_id", "work_request_id", "plan_revision", "agent_session", "issued_at", "expires_at", "request", "server_binding", "handoff", "state_binding", "phase_binding", "evaluation_context"])
    ...
    || JSON.stringify(value.plan_revision) !== JSON.stringify(passport.accepted_plan_revision) || !exactKeys(value.agent_session, ["id", "lease_expires_at"]) || !validId(value.agent_session.id) || !timestamp(value.agent_session.lease_expires_at)
    ...
    || state.canonical_record_digest !== passport.work_request.canonical_record_digest || !DIGEST.test(state.canonical_record_digest) || !list(state.accepted_resource_revisions) || state.accepted_resource_revisions.some((row) => !exactKeys(row, ["resource_ref", "revision_ref", "digest"]) || !string(row.resource_ref) || !string(row.revision_ref) || !DIGEST.test(row.digest)) || state.compare_and_swap_required !== true
