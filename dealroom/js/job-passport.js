...
function validExecutionEnvelope(value, passport) {
  if (!exactKeys(value, ["schema_version", "envelope_id", "work_request_id", "plan_revision", "agent_session", "issued_at", "expires_at", "request", "server_binding", "handoff", "state_binding", "phase_binding", "evaluation_context"])
    ...
    || JSON.stringify(value.plan_revision) !== JSON.stringify(passport.accepted_plan_revision) || !exactKeys(value.agent_session, ["id", "lease_expires_at"]) || !validId(value.agent_session.id) || !string(value.agent_session.lease_expires_at)
    || !string(value.issued_at) || !string(value.expires_at) || !exactKeys(value.request, ["job_ref", "input_digest", "data_class", "allowed_actions", "declared_expectations"]) || !validId(value.request.job_ref) || !DIGEST.test(value.request.input_digest)
    || value.request.data_class !== "synthetic_only" || !list(value.request.allowed_actions) || !exactKeys(value.request.declared_expectations, ["plan_step_refs", "component_refs", "component_dependencies", "resource_refs"])
    || !list(value.request.declared_expectations.plan_step_refs) || !value.request.declared_expectations.plan_step_refs.every(validId) || !list(value.request.declared_expectations.component_refs) || !value.request.declared_expectations.component_refs.every(validId)
    || !list(value.request.declared_expectations.resource_refs) || !value.request.declared_expectations.resource_refs.every(validId) || !list(value.request.declared_expectations.component_dependencies)
    || !object(value.server_binding) || !object(value.server_binding.identity) || !object(value.server_binding.authority) || !object(value.server_binding.adapter)
    || !exactKeys(value.handoff, ["mode", "replaces_agent_session_id", "capability_inherited", "checkpoint_ref", "native_session_transfer"]) || typeof value.handoff.capability_inherited !== "boolean"
    || !exactKeys(value.state_binding, ["state_version", "canonical_record_digest", "accepted_resource_revisions", "compare_and_swap_required"]) || !number(value.state_binding.state_version)
    || value.state_binding.state_version !== passport.work_request.state_version || value.state_binding.canonical_record_digest !== passport.work_request.canonical_record_digest || !DIGEST.test(value.state_binding.canonical_record_digest)
    || !list(value.state_binding.accepted_resource_revisions) || typeof value.state_binding.compare_and_swap_required !== "boolean" || !object(value.phase_binding) || !object(value.evaluation_context)) return false;
...
function validEngineeringPassport(value) {
  ...
  if (!receipts.every((receipt) => envelopeDigests.has(receipt.envelope_digest))) return false;
