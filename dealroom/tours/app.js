(() => {
  ...
  const state = { csrf: "", tours: [], tour: null, rawShareToken: "", shareGrantId: "", projectionId: "", projectionDraftId: "", candidateDigest: "", renderJobId: "", pdfQcRunDigest: "" };
  ...
  function renderTour() {
    ...
    state.projectionId = text(tour.projection_id); state.projectionDraftId = text(tour.projection_draft_id); state.shareGrantId = text(tour.share_grant_id);
    $("#projection-state").textContent = state.projectionId ? "Approved" : tour.projection_status === "draft" ? "Draft · approval required" : "Not generated"; $("#share-state").textContent = state.shareGrantId ? "Active" : "Not issued";
  ...
  async function loadTour(tourId) { status("Loading tour…"); state.tour = await request(`/api/tours/detail?tour_id=${encodeURIComponent(tourId)}`); renderTour(); await loadProjectionPreview(); status("Tour ready."); }
